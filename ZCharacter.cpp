...
//	RegisterModules();
	...
	m_pModule_GoldDamage		= new ZModule_GoldDamage;
...
bool ZCharacter::IsAdminName()
{
	if(m_MInitialInfo.Ref().nUGradeID == MMUG_DEVELOPER ||
		m_MInitialInfo.Ref().nUGradeID == MMUG_OWNER ||
...
// ISGrade for Scoreboard
bool ZCharacter::IsGrade255()
{
	if(m_MInitialInfo.Ref().nUGradeID == MMUG_OWNER) return true;
	return false;
}
bool ZCharacter::IsGrade254()
{
	if(m_MInitialInfo.Ref().nUGradeID == MMUG_GAMEMASTER) return true;
	return false;
}
bool ZCharacter::IsGrade253()
{
	if(m_MInitialInfo.Ref().nUGradeID == MMUG_BLOCKED) return true;
	return false;
}
bool ZCharacter::IsGrade252()
{
	if(m_MInitialInfo.Ref().nUGradeID == MMUG_DEVELOPER) return true;
	return false;
}
bool ZCharacter::IsGrade251()
{
	if(m_MInitialInfo.Ref().nUGradeID == MMUG_ADMINISTRATOR) return true;
	return false;
}
bool ZCharacter::IsGrade250()
{
	if(m_MInitialInfo.Ref().nUGradeID == MMUG_FMOD) return true;
	return false;
}



bool ZCharacter::IsDonateName1()
{
	if(m_MInitialInfo.Ref().nUGradeID == MMUG_DONATE1)
	{
		return true;
	}
	return false;
}
bool ZCharacter::IsDonateName2()
{
	if(m_MInitialInfo.Ref().nUGradeID == MMUG_DONATE2)
	{
		return true;
	}
	return false;
}
bool ZCharacter::IsDonateName3()
{
	if(m_MInitialInfo.Ref().nUGradeID == MMUG_DONATE3)
	{
		return true;
	}
	return false;
...
void ZCharacter::OnDamaged(ZObject* pAttacker, rvector srcPos, ZDAMAGETYPE damageType, MMatchWeaponType weaponType, float fDamage, float fPiercingRatio, int nMeleeType)
{
	...
	if ((stageSetting->IsNoLead()) && ((damageType == ZD_BULLET) || (damageType == ZD_BULLET_HEADSHOT)) && (stageSetting->GetGameType() != MMATCH_GAMETYPE_QUEST) &&  (stageSetting->GetGameType() != MMATCH_GAMETYPE_SURVIVAL) && (pAttacker == ZGetGame()->m_pMyCharacter) && (this != ZGetGame()->m_pMyCharacter)) 
	{
		void* pBlobArray = MMakeBlobArray(sizeof(MTD_RangeInfo), 1);
		...
		Info.fPosY = srcPos.y;
		Info.fPosZ = srcPos.z;
		Info.nDamageType = damageType;
		Info.fRatio = fPiercingRatio;
		Info.nLowId = GetUID().Low;
		Info.nWeaponType = weaponType;

		memcpy(pBlobElement, &Info, sizeof(MTD_RangeInfo));
		ZGetGameClient()->GetPeerPacketCrypter().Encrypt((char*)pBlobElement, sizeof(MTD_RangeInfo));

		ZPOSTCMDLEAD(MC_PEER_NOLEAD_RANGE, GetUID(), MCommandParameterBlob(pBlobArray, MGetBlobArraySize(pBlobArray)));
		fDamage = 0;    
	}

	if ((stageSetting->IsNoLead()) && ((damageType == ZD_MELEE) || (damageType == ZD_KATANA_SPLASH)) && (stageSetting->GetGameType() != MMATCH_GAMETYPE_QUEST) &&  (stageSetting->GetGameType() != MMATCH_GAMETYPE_SURVIVAL) && (pAttacker == ZGetGame()->m_pMyCharacter) && (this != ZGetGame()->m_pMyCharacter))
	{
		void* pBlobArray = MMakeBlobArray(sizeof(MTD_MeleeInfo), 1);
		void* pBlobElement = MGetBlobArrayElement(pBlobArray, 0);
		...
		memcpy(pBlobElement, &Info, sizeof(MTD_MeleeInfo));
		ZGetGameClient()->GetPeerPacketCrypter().Encrypt((char*)pBlobElement, sizeof(MTD_MeleeInfo));

		ZPOSTCMDLEAD(MC_PEER_NOLEAD_MELEE, ZGetGameClient()->GetPlayerUID(), MCommandParameterBlob(pBlobArray, MGetBlobArraySize(pBlobArray)));
		fDamage = 0;    
	}
