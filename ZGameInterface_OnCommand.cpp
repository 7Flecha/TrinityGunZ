	...
	case MC_MATCH_RESPONSE_SELECT_CHAR:
		{
			...
			if (nResult == MOK)
			{
				...
				*/
				...
				if ((ZGetGameClient()->GetServerMode() != MSM_CLAN) || (ZGetGameClient()->GetServerMode() != MSM_CLAN) || (!ZGetMyInfo()->IsClanJoined()))	
				{
	...
	case MC_MATCH_RESPONSE_CHARACTER_ITEMLIST:
		{
			...
			pCommand->GetParameter(&nVipCoins, 1, MPT_INT);
			ZGetMyInfo()->SetVipCoins(nVipCoins);
