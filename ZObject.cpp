...
void ZObject::OnDamaged(ZObject* pAttacker, rvector srcPos, ZDAMAGETYPE damageType, MMatchWeaponType weaponType, float fDamage, float fPiercingRatio, int nMeleeType)
{
	...
	if(ZGetSGame()->bGodMode == true && damageType != ZD_FALLING)
	{
		if (pAttacker == ZGetGame()->m_pMyCharacter) 
		{
			float fpNewHP = (((ZCharacter *)pAttacker)->GetHP() + fDamage);
			float fpNewAP = (((ZCharacter *)pAttacker)->GetAP() + fDamage);
	...
	if (ZGetGame()->GetMatch()->GetMatchType() == MMATCH_GAMETYPE_VAMPIRE && pAttacker == ZGetGame()->m_pMyCharacter && damageType != ZD_FALLING) 
	{
		float fpNewHP = (((ZCharacter *)pAttacker)->GetHP() + UPDATE_HEALTH_HPV);
		float fpNewAP = (((ZCharacter *)pAttacker)->GetAP() + UPDATE_HEALTH_APV);
