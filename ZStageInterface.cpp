...
while(1){
	...
	if(LastMouseMoveTime + 30000 < timeGetTime() && WarningOutput != false){
		ZChatOutput(ZCOLOR_CHAT_SYSTEM_GAME, "Advertencia: Si no se mueve el raton en 30 segundos, volvera al lobby");
		WarningOutput = false;
...
void ZStageInterface::OnCreate( void)
{
	...
	m_pChallengeQuestScenario->ReadXml(ZApplication::GetFileSystem(), "localized/localize_english/scenario2.xml", ZGetQuest()->GetDropTable());
	...
	if ( pPicture)
	{
		...
		((MBitmapR2*)m_pStageFrameImg)->Create( "stage_frame.png", RGetDevice(), "common/interface/loadable/stage_frame.png");
	...
	if ( pPicture)
	{
		...
		((MBitmapR2*)m_pItemListFrameImg)->Create( "itemlistframe.tga", RGetDevice(), "common/interface/loadable/itemlistframe.tga");
	...
	{ // 릴레이맵 "RelayMap" 초기화
		...
		{ // f릴레이맵 리스트 배경 이미지
			...
			((MBitmapR2*)m_pRelayMapListFrameImg)->Create( "relaymaplistframe.tga", RGetDevice(), "common/interface/loadable/relaymaplistframe.tga");
...
*/
	...
	if ( pPicture)
	{
		...
		{
			sprintf( szMapName, "common/interface/loadable/%s", MGetMapDescMgr()->GetMapImageName( MMATCH_DEFAULT_STAGESETTING_MAPNAME));
		}
		else
			sprintf( szMapName, "common/interface/loadable/%s", MGetMapDescMgr()->GetMapImageName( ZGetGameClient()->GetMatchStageSetting()->GetMapName()));
...
sprintf( szStr, "[%03d] %s", ZGetGameClient()->GetStageNumber(), ZGetGameClient()->GetStageName());
...
void ZStageInterface::SetStageRelayMapImage()
{
	...
	{
		sprintf( szMapName, "common/interface/loadable/%s", MGetMapDescMgr()->GetMapImageName( pRelayMapListBox->GetString(pRelayMapListBox->GetStartItem())));
		if ( m_pTopBgImg != NULL)
		{
			delete m_pTopBgImg;
			m_pTopBgImg = NULL;
...
void ZStageInterface::ChangeStageGameSetting( const MSTAGE_SETTING_NODE* pSetting)
{
	...
	if ( (pSetting->nGameType == MMATCH_GAMETYPE_DEATHMATCH_SOLO) ||			// 데쓰매치 개인전이거나...
		 ...
		 (pSetting->nGameType == MMATCH_GAMETYPE_VAMPIRE))				// 암살전 이면...
	{
		// 맵 이름 배경 이미지 변환
		if ( pAniMapImg)
	...
	else if ( pSetting->nGameType == MMATCH_GAMETYPE_SURVIVAL || pSetting->nGameType == MMATCH_GAMETYPE_QUEST_CHALLENGE)	
	{
		// 맵 이름 배경 이미지 변환
		if ( pAniMapImg)
			pAniMapImg->SetCurrentFrame( 0);
	...
	else if (pSetting->nGameType == MMATCH_GAMETYPE_QUEST_CHALLENGE)	
	{
		// 맵 이름 배경 이미지 변환
		if ( pAniMapImg)
			pAniMapImg->SetCurrentFrame( 0);
	...
	else if ( pSetting->nGameType == MMATCH_GAMETYPE_QUEST )				// 서바이벌 모드이면...
...
//				ZApplication::GetGameInterface()->EnableWidget( "StageMaxPlayer", true);
	...
	if ( (pSetting->nGameType == MMATCH_GAMETYPE_SURVIVAL) || (pSetting->nGameType == MMATCH_GAMETYPE_QUEST) || (pSetting->nGameType == MMATCH_GAMETYPE_QUEST_CHALLENGE))
		ZApplication::GetGameInterface()->EnableWidget( "StageSettingCaller", false);
...
bool ZStageInterface::ReadSenarioNameXML( void)
{
	...
	string strFileScenario("localized/localize_english/scenario.xml");
