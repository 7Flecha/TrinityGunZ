	...
	while(true){
		...
		if(nPos + 1 < strlen(pSrc)){
			//컬러 지정
				...
				if(pSrc[nPos+1] >= 48 && pSrc[nPos+1] <= 57)
				{
					SetColor(MCOLOR(MMColorSet[pSrc[nPos+1] - '0']));
				}
				else if(pSrc[nPos+1] >= 97 && pSrc[nPos+1] <= 122)
				{
					SetColor(MCOLOR(MMColorSet[pSrc[nPos+1] - '10']));
				}
				pSrc = pSrc + nPos + 2;
			} else {
				Text(x+nOffset, y, "^");
...
#define FLUSH				if(buffer[0]) { Text(r.x+nLastX, y, buffer); FLUSHPOS(r.x+nLastX); nLastX=nX; buffer[0]=0;pcurbuf=buffer; }
				...
				for(int i=0; i<nCharCount; i++){
					...
					if((c=='^' && ('0'<=cc) && (cc<='9')) || (c=='^' && ('a'<=cc) && (cc<='z')))
					{
						FLUSH;
						if(('0'<=cc) && (cc<='9'))
							SetColor(MCOLOR(MMColorSet[cc - '0']));
						else
							SetColor(MCOLOR(MMColorSet[int(cc) - 'W']));
						i++;
						continue;
					}
