...
//inline void ZPostLoginRegister(char* szUserID, char* szPassword, unsigned int nChecksumPack, char* szName, char* szAge, char* szEmail, char *szEncryptMD5Value)
//{
//	void *pBlob = MMakeBlobArray( MAX_MD5LENGH, 1 );
//	unsigned char *pCmdBlock = (unsigned char*)MGetBlobArrayElement(pBlob, 0);
//	CopyMemory( pCmdBlock, szEncryptMD5Value, MAX_MD5LENGH );
//	
//	ZPOSTCMD8(MC_MATCH_LOGIN_REGISTER, MCmdParamStr(szUserID)
//		, MCmdParamStr(szPassword)
//		, MCmdParamInt(MCOMMAND_VERSION)
//		, MCmdParamUInt(nChecksumPack)
//		, MCmdParamStr(szName)
//		, MCmdParamStr(szAge)
//		, MCmdParamStr(szEmail)
//		, MCommandParameterBlob(pBlob, MGetBlobArraySize(pBlob)));
//	MEraseBlobArray(pBlob);
//}
...
{
	ZPOSTCMD2(MC_MATCH_CHATROOM_CREATE, MCmdParamUID(uidPlayer),MCmdParamStr(pszChatRoomName));
}
...
inline void ZPostLadderRequestChallenge(char** ppMemberCharNames, const int nMemberCount, unsigned long int nOptions)	// 자신까지 포함
{
	...
	ZPOSTCMD3( MC_MATCH_LADDER_REQUEST_CHALLENGE, MCmdParamInt(nMemberCount), MCmdParamUInt(nOptions), MCmdParamBlob(pBlobMembersName, MGetBlobArraySize(pBlobMembersName)));
...
//chellangequest
inline void ZPostNewQuestGameKill()
{
	ZPOSTCMD0(MC_NEWQUEST_REQUEST_DEAD);
}
...
{
    ZPOSTCMD1(MC_ADMIN_BAN, MCmdParamStr(pszTargetName));
}
