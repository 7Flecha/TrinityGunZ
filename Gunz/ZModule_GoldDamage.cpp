...
ZModule_GoldDamage::ZModule_GoldDamage()
{
	m_bOnDamage = false;
	m_pOwner = NULL;
}

void ZModule_GoldDamage::InitStatus()
{
	m_bOnDamage = false;
	Active(false);
}

bool ZModule_GoldDamage::Update(float fElapsed)
{
	...
	else
	{
		...
		if(ZGetGame()->GetTime()>m_fNextDamageTime) {
			...
			else //살아있을떄만..
			{
				...
				pObj->OnDamaged(m_pOwner, pObj->GetPosition(), ZD_GOLD, MWT_NONE, m_fDamage, 0);
		...
		if(ZGetGame()->GetTime()>m_fNextEffectTime) {
			...
			if(!pObj->IsDie())
			{
				...
				ZGetEffectManager()->AddEnchantGold2( pObj );
				ZGetEffectManager()->AddEnchantGold2( pObj );
			}
		}
	}
...
// 데미지를 주기 시작한다
void ZModule_GoldDamage::BeginDamage(ZObject* pOwner, int nDamage, float fDuration)
{
