...
bool ZScreenEffect::DrawCustom(unsigned long int nTime, rvector& vOffset, float fAngle)
{
	...
	else if( RGetIsHDScreen() )
	{
		const rvector eye(0,0,-866),at(0,0,0),up(0,1,0);
		D3DXMatrixLookAtLH(&View,&eye,&at,&up);
	}
...
#define END_(x) { _end_time = timeGetTime(); float f_time = (_end_time - _begin_time) / 1000.f; mlog("%s : %f \n", x,f_time ); }
	...
	if(m_pQuestEffectMeshMgr->LoadXmlList("common/interface/default/combat/screeneffects_quest.xml")==-1) {
	...
	if(m_pEffectMeshMgr->LoadXmlList("common/interface/default/combat/screeneffects.xml")==-1) {
	...
	m_pGaugeTexture=RCreateBaseTexture("common/Interface/Default/COMBAT/gauge.bmp");
...
void ZScreenEffectManager::Draw()
{
	...
	if(ZGetCombatInterface()->IsShowUI())
	{
		DrawCTFEffects();		// For CTF Arrow / Text
		DrawQuestEffects();		// For Quest Interface
		DrawNewQuestEffects();	// For ChallengeQuest Interface
		DrawDuelEffects();		// Duel Effects
		DrawTDMEffects();		// Team Duelmatch Interface
	}
}
...
void ZScreenEffectManager::UpdateEffects()
{
	...
	if ((!ZGetGameTypeManager()->IsQuestDerived(ZGetGameClient()->GetMatchStageSetting()->GetGameType()) || ((!ZGetGameClient()->GetMatchStageSetting()->GetGameType() == MMATCH_GAMETYPE_QUEST_CHALLENGE )))) return;
	if (m_pBossHPPanel)
	{
...
void ZScreenEffectManager::DrawCTFEffects()
{
	...
	ZCharacter* pRed =(ZCharacter*) ZGetCharacterManager()->Find(pTeamCTF->GetRedCarrier());
	ZCharacter* pBlue =(ZCharacter*) ZGetCharacterManager()->Find(pTeamCTF->GetBlueCarrier());
	ZCharacter* pCharacter = ZGetGame()->m_pMyCharacter;
	ZCharacter* pUID =(ZCharacter*) ZGetCharacterManager()->Find(pCharacter->GetUID());

	m_pArrow = new ZScreenEffect(m_pEffectMeshMgr->Get("arrow"));

	if(pCharacter->GetTeamID() == MMT_RED)
	{
		if(pTeamCTF->GetRedFlagState() == true)
		{
			if(pBlue == pUID)
			{
				if(pTeamCTF->GetBlueFlagState() == true)
				{
					rvector to = pRed->GetPosition();
					DrawArrow(to);
				}
				else
				{
					rvector to = pTeamCTF->GetRedFlagPos();
					DrawArrow(to);
				}
			}
			else
			{
				rvector to = pBlue->GetPosition();
				DrawArrow(to);
			}
	...
	else if(pCharacter->GetTeamID() == MMT_BLUE)
	{
		if(pTeamCTF->GetBlueFlagState() == true)
		{
			if(pRed == pUID)
			{
				...
				{
					rvector to = pTeamCTF->GetBlueFlagPos();
					DrawArrow(to);
				}
			}
			else
...
void ZScreenEffectManager::DrawNewQuestEffects()
{
if (!ZGetGameTypeManager()->IsCQuestDerived(ZGetGameClient()->GetMatchStageSetting()->GetGameType())) return;
...
void ZScreenEffectManager::DrawArrow(rvector& vTargetPos)
{
	...
	if (ZGetGameClient()->GetMatchStageSetting()->GetGameType() == MMATCH_GAMETYPE_CTF)
	{
		...
		const float fOffSetZ = 50.0f;
		if (m_pArrow)
		{
			m_pArrow->DrawCustom(0, rvector(0.0f, fOffsetY, fOffSetZ), fAng);
		}
	}
	else
	{
