...
void ZMatch::ProcessRespawn()
{
	...
	if (ZGetGameClient()->GetMatchStageSetting()->GetGameType() == MMATCH_GAMETYPE_QUEST_CHALLENGE) return;
#endif
	// µ¥¾²¸ÅÄ¡ÀÏ °æ¿ì Á×À¸¸é ¹Ù·Î »ì¾Æ³­´Ù.
	if (!IsWaitForRoundEnd() && ZGetGame()->m_pMyCharacter)
	{
		...
		{
			if (bLastDead == false) {
				m_nLastDeadTime = m_nNowTime;	// Á×Àº ½Ã°£
			}
			...
			if (m_nSoloSpawnTime >= RESPAWN_DELAYTIME_AFTER_DYING_MAX)	// Á×¾î¼­ ¾Æ¹«°Íµµ ¾È´©¸£°í ÀÖÀ¸¸é 15ÃÊÈÄ ¸®½ºÆù
			{
				SoloSpawn();
			}
...
void ZMatch::InitCharactersPosition()
{
	...
	else if ( ZGetGame()->GetMatch()->GetMatchType() == MMATCH_GAMETYPE_DUEL)
	{
		...
		if ( pDuel)
		{
			...
			// µà¾ó Àü¿ë ¸ÊÀÌ¸é ¸Ç Ã³À½°ú µÎ¹øÂ° À§Ä¡
			...
			// µà¾ó Àü¿ë ¸ÊÀÌ ¾Æ´Ï¸é ÆÀÀü°ú °°Àº ¹æ½Ä
			else
			{
...
//	m_nRoundKills = 0;
	...
	ZGetGame()->m_pMyCharacter->GetStatus().Ref().nDamageGekregen = 0;
	ZGetGame()->m_pMyCharacter->GetStatus().Ref().nDamageGemaakt = 0;
...
void ZMatch::SetRoundState(MMATCH_ROUNDSTATE nRoundState, int nArg)
{
	...
	//# ¾ÕÀ¸·Î´Â ÀÌ switch ³»ºÎ¿¡ ·ê¿¡ µû¸¥ if else if else¸¦ Ãß°¡ÇÏÁö ¾Ê´Â°Ô ÁÁ°Ú½À´Ï´Ù.
	//# ¶ó¿îµå»óÅÂ º¯È­½Ã ·ê¿¡ µû¶ó ÇØ¾ßÇÒ ÀÏÀº ZRule***ÂÊ¿¡ ³Ö´Â °ÍÀÌ ÁÁ°Ú½À´Ï´Ù. => virtual ZRule::OnSetRoundState()
...
void ZMatch::OnForcedEntry(ZCharacter* pCharacter)
{
	...
	{
		// ÆÀÇÃ·¹ÀÌ¿¡ ´Ù¸¥ ÇÃ·¹ÀÌ¾î°¡ ³­ÀÔÇÏ¸é ´ÙÀ½ÆÇºÎÅÍ ÁøÇàÇØ¾ß ÇÏ±â¶§¹®¿¡ Ä³¸¯ÅÍ º¸¿©ÁÖÁö ¾ÊÀ½
		// ´Ü FREE»óÅÂÀÏ ¶§¿¡´Â º¸¿©ÁØ´Ù
