...
RRESULT OnCreate(void *pParam)
{
	...
	string strFileLenzFlare("localized/localize_english/LenzFlare.xml");
	...
	RBspObject::CreateShadeMap("common/sound_sfx/water_splash.bmp");
	//D3DCAPS9 caps;
...
//	if( !g_pDefFont->Create("Default", RGetDevice(), "FONTb11b", 14, 1.0f, true, false) )
	...
	{
		ZGetInitialLoading()->AddBitmap( 0, "Common/Interface/Default/LOADING/Gunz2_BackGround.tga" );
		ZGetInitialLoading()->AddBitmapBar( "Common/Interface/Default/LOADING/Gunz2_Bar.tga" );
//		ZGetInitialLoading()->AddBitmapLight( "Common/Interface/Default/LOADING/Gunz2_Light.png" );
...
//loading Herogamers gunz
//	if( ZGetInitialLoading()->IsUseEnable() )
//	{
//		// Loading Multi 1
//		ZGetInitialLoading()->AddBitmap( 0, "common/Interface/Default/LOADING/Loading_1.tga" );
//		ZGetInitialLoading()->AddBitmapBar( "common/Interface/Default/LOADING/Loading_bar.jpg" );
//		ZGetInitialLoading()->SetText( g_pDefFont, 10, 30, cstrReleaseDate );
//		ZGetInitialLoading()->SetPercentage( 0.0f );
...
#ifndef _PUBLISH //Was _DEBUG to disable mef files
...
void UpgradeMrsFile()
{
//Nick: Delete Update files
//	char temp_path[ 1024];
//	sprintf( temp_path,"*");
//
//	FFileList file_list;
//	GetFindFileListWin(temp_path,".dev",file_list);
...
int PASCAL WinMain(HINSTANCE this_inst, HINSTANCE prev_inst, LPSTR cmdline, int cmdshow)
{
	...
	// ¿©±â¼­ ASSERT ½ÇÆÐ°¡ ¶á °æ¿ì, Ãâ·ÂÃ¢¿¡¼­ test error ¸Þ½ÃÁö¸¦ Ã£¾Æ ´õºíÅ¬¸¯ÇÏ¸é ½ÇÆÐÇÑ Å×½ºÆ®¸¦ º¼ ¼ö ÀÖ½À´Ï´Ù.

	// ±×³É return 0; ÇØ¹ö¸®¸é ¸Þ¸ð¸®Ç® ÇØÁ¦°¡ ½ÇÇàµÇÁö ¾Ê¾Æ leakÀÌ ³³´Ï´Ù. ÀÏ´Ü ÀÓ½Ã·Î ¼öµ¿ ÇØÁ¦ Çß½À´Ï´Ù.
	// ÀÌ·± ¼öµ¿ ÇØÁ¦°¡ ¸¹ÀÌ ÇÊ¿äÇØÁö¸é ¸®ÆÑÅä¸µÀ» ÇØ¾ß°Ú½À´Ï´Ù.
	ZBasicInfoItem::Release();	
	
	// ´ÙÀ½ ½ÇÇàÀÎ¼ö°¡ ÀÖÀ¸¸é Å×½ºÆ®µé¸¸ ¼öÇàÇÏ°í ³¡³À´Ï´Ù
	if (0==stricmp(cmdline, "/UnitTest"))
		return 0;

	// test suite ÄÚµå¿¡¼­ ¹ß»ýÇÑ ¸Þ¸ð¸® ¸¯À» Ã£°í ½ÍÀ¸¸é ¾Æ·¡ ÇàÀ» ÁÖ¼® Ç®°í ½ÇÇàÇÏ¼¼¿ä
	//return 0;
#endif
...
#define AHNHS_CHKOPT_GUNZ		AHNHS_CHKOPT_ALL
...
#ifndef _DEBUG
	// ¾Æ·¡ ÄÚµå´Â ¸±¸®Áî½Ã _AhnHS_Initialize ¿É¼ÇÀÌ AHNHS_CHKOPT_ALL·Î µÇ¾îÀÖÁö ¾ÊÀ» °æ¿ì 
	// ÄÄÆÄÀÏ Å¸ÀÓ¿¡ ¿¡·¯¸¦ ÀÏÀ¸ÄÑ¼­ ¾Ë ¼ö ÀÖ°Ô ÇÑ´Ù.
	...
	//ÀÌÈÄ °áÄÚ ¹ß»ýÇØ¼­´Â ¾ÈµÇ´Â ¿¡·¯ÀÌ¹Ç·Î assertÃ³¸®¸¦ Çß½À´Ï´Ù.
...
//				MessageBox(NULL, _T("´Ù¸¥ °ÔÀÓÀÌ ½ÇÇàÁßÀÔ´Ï´Ù.\nÇÁ·Î±×·¥À» Á¾·áÇÕ´Ï´Ù."), szTitle, MB_OK);
				mlog( "´Ù¸¥ °ÔÀÓÀÌ ½ÇÇàÁßÀÔ´Ï´Ù. ÇÁ·Î±×·¥À» Á¾·áÇÕ´Ï´Ù.\n");
				break;
			}
		case HS_ERR_INVALID_FILES:
			{
//				MessageBox(NULL, _T("Àß¸øµÈ ÆÄÀÏ ¼³Ä¡µÇ¾ú½À´Ï´Ù.\nÇÁ·Î±×·¥À» Àç¼³Ä¡ÇÏ½Ã±â ¹Ù¶ø´Ï´Ù."), szTitle, MB_OK);
				mlog( "Àß¸øµÈ ÆÄÀÏ ¼³Ä¡µÇ¾ú½À´Ï´Ù. ÇÁ·Î±×·¥À» Àç¼³Ä¡ÇÏ½Ã±â ¹Ù¶ø´Ï´Ù.\n");
				break;
			}
		case HS_ERR_DEBUGGER_DETECT:
			{
//				MessageBox(NULL, _T("ÄÄÇ»ÅÍ¿¡¼­ µð¹ö°Å ½ÇÇàÀÌ °¨ÁöµÇ¾ú½À´Ï´Ù.\nµð¹ö°ÅÀÇ ½ÇÇàÀ» ÁßÁö½ÃÅ² µÚ¿¡ ´Ù½Ã ½ÇÇà½ÃÄÑÁÖ½Ã±â¹Ù¶ø´Ï´Ù."), szTitle, MB_OK);
				mlog( "ÄÄÇ»ÅÍ¿¡¼­ µð¹ö°Å ½ÇÇàÀÌ °¨ÁöµÇ¾ú½À´Ï´Ù. µð¹ö°ÅÀÇ ½ÇÇàÀ» ÁßÁö½ÃÅ² µÚ¿¡ ´Ù½Ã ½ÇÇà½ÃÄÑÁÖ½Ã±â¹Ù¶ø´Ï´Ù.\n");
				break;
			}
		case HS_ERR_NEED_ADMIN_RIGHTS:
			{
//				MessageBox(NULL, _T("Admin ±ÇÇÑÀ¸·Î ½ÇÇàµÇ¾î¾ß ÇÕ´Ï´Ù.\nÇÁ·Î±×·¥À» Á¾·áÇÕ´Ï´Ù."), szTitle, MB_OK);
				mlog( "Admin ±ÇÇÑÀ¸·Î ½ÇÇàµÇ¾î¾ß ÇÕ´Ï´Ù. ÇÁ·Î±×·¥À» Á¾·áÇÕ´Ï´Ù.\n");
				break;
			}
		case HS_ERR_COMPATIBILITY_MODE_RUNNING:
			{
//				MessageBox(NULL, _T("È£È¯¼º ¸ðµå·Î ÇÁ·Î±×·¥ÀÌ ½ÇÇàÁßÀÔ´Ï´Ù.\nÇÁ·Î±×·¥À» Á¾·áÇÕ´Ï´Ù."), szTitle, MB_OK);
				mlog( "È£È¯¼º ¸ðµå·Î ÇÁ·Î±×·¥ÀÌ ½ÇÇàÁßÀÔ´Ï´Ù. ÇÁ·Î±×·¥À» Á¾·áÇÕ´Ï´Ù.\n");
				break;				
			}
		default:
			{
				TCHAR szMsg[255];
				wsprintf(szMsg, _T("ÇØÅ·¹æÁö ±â´É¿¡ ¹®Á¦°¡ ¹ß»ýÇÏ¿´½À´Ï´Ù.(Error Code = %x)\nÇÁ·Î±×·¥À» Á¾·áÇÕ´Ï´Ù."), nRet);
//				MessageBox(NULL, szMsg, szTitle, MB_OK);
	...
	{
//		TCHAR szMsg[255];
		mlog(_T("ÇØÅ·¹æÁö ±â´É¿¡ ¹®Á¦°¡ ¹ß»ýÇÏ¿´½À´Ï´Ù.(Error Code = %x)\nÇÁ·Î±×·¥À» Á¾·áÇÕ´Ï´Ù."), nRet);
//		MessageBox(NULL, szMsg, szTitle, MB_OK);

		return FALSE;
	}

	nRet = _AhnHS_SaveFuncAddress(6, _AhnHS_Initialize, _AhnHS_StartService,			// ÇÙ½Çµå ÇÔ¼ö
										ZCheckHackProcess, ZGetMZFileChecksum,				// ÇÙÆÇº° ÀÚÃ¼ÇÔ¼ö
...
#if defined(LOCALE_BRAZIL) || defined(LOCALE_BETA) || defined(LOCALE_INDIA) || defined(LOCALE_US) || defined(LOCALE_KOREA)
	//#ifndef _DEBUG
	#ifdef _PUBLISH
		// GunzLockÀ» ¶ç¿ö³õ°EGunz.exe¸¦ ½ÇÇàÇÏ¸EÁ¾·áÁ÷ÀE´EâÇÑ´Ù. (XProtector ÇÁ·Î¼¼½ºÀÌ¹ÌÁö½ºÄµ ÀÛ¾÷¿E //LOCK/MUTEX
...
*/
	...
	sprintf( szDumpFileName, "GunzBeta_%d-%d-%d_%d-%d-%d.dmp"
	, SystemTime.wYear
	, SystemTime.wMonth
	, SystemTime.wDay
	, SystemTime.wHour
	...
	if (ZApplication::GetInstance()->ParseArguments(cmdline) == false)
	{
		...
		if ((ZGetLocale()->GetCountry() == MC_JAPAN) || (ZGetLocale()->GetCountry() == MC_JAPAN))
		{
...
//	while(ShowCursor(FALSE)>0);
	...
	if( 0 != RInitD3D(&g_ModeParams) )
	{
		...
		{
			ShellExecute(NULL, "open", "http://www.microsoft.com/download/en/details.aspx?id=35", NULL, NULL, SW_SHOWNORMAL);
		}
