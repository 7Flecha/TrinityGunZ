...
#define END_(x) { _end_time = timeGetTime(); float f_time = (_end_time - _begin_time) / 1000.f; mlog("%s : %f \n", x,f_time ); }
	...
	bool bRet = true;
	char szPath[256];
	char szSubPath[256] = "common/interface/grm/default/";	// 언어변경을 위한 대체 경로
	char szFileName[256];
...
void ZGameInterface::EnableLobbyInterface(bool bEnable)
{
	...
	EnableWidget("LobbyOptionFrame_Lobby", bEnable);			// 옵션 버튼
	...
	EnableWidget("StageCreateFrameCaller_Extra", bEnable);	// 게임생성 버튼

	EnableWidget("ShopCaller", bEnable);				// 상점 버튼
	EnableWidget("EquipmentCaller", bEnable);			// 장비 버튼
	EnableWidget("EquipmentCaller_Lobby", bEnable);			// 장비 버튼
