...
struct ZScoreBoardItem : public CMemPoolSm<ZScoreBoardItem>{
	...
	ZScoreBoardItem( const MUID& _uidUID, char* _szLevel, char *_szName, char *_szGrade, char *_szClan,int _nTeam,bool _bDeath,int _nExp,int _nKills,int _nDeaths,int _nPing,int _nDTLastWeekGrade, bool _bMyChar,bool _bGameRoomUser, bool _bCommander = false)//ZZZZZZ
	{
		uidUID=_uidUID;
		strcpy(szLevel,_szLevel);
		strcpy(szName,_szName);
		strcpy(szGrade,_szGrade);
...
bool ZCombatInterface::OnCreate()
{
	...
	if (ZGetMyInfo()->IsAdminGrade()) {
		...
		if (pCache && pCache->GetUGrade()==MMUG_ADMINISTRATOR && pCache->CheckFlag(MTD_PlayerFlags_AdminHide)) {
...
void ZCombatInterface::DrawExtraEffects(MDrawContext* pDC)
{

	if(ZGetConfiguration()->GetExtra()->bDisplayFPS == 1) 
	{
		...
		char DrawFPS[128];
		sprintf(DrawFPS, "FPS: %.f",g_fFPS);
		TextRelative(pDC, 155.f/800.f, 15.f/600.f,DrawFPS);
	}

	if(ZGetConfiguration()->GetExtra()->bDisplayHPAP == 1)
	{
...
#define CHARACTER_HEIGHT	185.0f
				...
				*/
				...
				{	
					// Team Color Add 8-1-2013 by George Edit By Nick
					MFont *pFont=GetGameFont();
					if(ZGetGame()->m_pMyCharacter->GetTeamID() == MMT_BLUE)
					{
						pFont=MFontManager::Get("FONTa12_O1Blr");
						pDC->SetColor(MCOLOR(0xFF080AB7));
					}
					else if(ZGetGame()->m_pMyCharacter->GetTeamID()== MMT_RED)
					{
						pFont=MFontManager::Get("FONTa12_O1Red");
						pDC->SetColor(MCOLOR(0xFFA21100));
					}
					else
					{
						pFont = MFontManager::Get("FONTa12_O1Blr");
...
*/
	...
	{
		sprintf( szText, "Time Passed : 0:00");
		TextRelative( pDC, x, y, szText);
		y -= linespace2;
	}
	...
	{
		sprintf( szText, "%s : %d / %d %s", ZMsg( MSG_WORD_RPROGRESS), ZGetGame()->GetMatch()->GetCurrRound() + 1, ZGetGame()->GetMatch()->GetRoundCount(), "Round");
	}
...
//	const float clanXPOS[6] = {0.45f, 0.25f, 0.68f, 0.77f, 0.83f, 0.92f};
	...
	// Rank
	x = ITEM_XPOS[2] - .01f;	
	TextRelative(pDC,x,y, "Rank");

	// Points
	x = ITEM_XPOS[4]-0.03f;
	TextRelative(pDC,x,y, "Points");
...
//	int nMaxLineCount=int((fHeight-fTitleHeight)*(float)RGetScreenHeight()/((float)pFont->GetHeight()*1.1f));
		...
		itor != ZGetCharacterManager()->end(); ++itor)
	{
		...
		else if(pCharacter->IsDonateName1()) {
			pItem->SetColor(ZCOLOR_DONATE_NAME1);
			sprintf(pItem->szLevel,"%d%s",pCharacter->GetProperty()->nLevel, ZMsg(MSG_CHARINFO_LEVELMARKER));
		}
		else if(pCharacter->IsDonateName2()) {
			pItem->SetColor(ZCOLOR_DONATE_NAME2);
			sprintf(pItem->szLevel,"%d%s",pCharacter->GetProperty()->nLevel, ZMsg(MSG_CHARINFO_LEVELMARKER));
		}
		else if(pCharacter->IsDonateName3()) 
		{
		unsigned long Color[] = {0xFFFF00FF, 0xFF8B008B, 0xFF6969CD, 0xFF963EFF, 0xFFEE00EE, 0xFF8B008B };
			...
			//pItem->SetColor(ZCOLOR_DONATE_NAME3);
			//sprintf(pItem->szLevel,"%d%s",pCharacter->GetProperty()->nLevel, ZMsg(MSG_CHARINFO_LEVELMARKER));
		}
		else{
			sprintf(pItem->szLevel,"%d%s",pCharacter->GetProperty()->nLevel, ZMsg(MSG_CHARINFO_LEVELMARKER));
		...
		if(pCharacter->IsGrade255())
			sprintf(pItem->szGrade,"%s","Owner");
		else if(pCharacter->IsGrade254())
			sprintf(pItem->szGrade,"%s","GM");
		else if(pCharacter->IsGrade253()) 
				sprintf(pItem->szGrade,"%s","Banned");
		else if(pCharacter->IsGrade252()) 
				sprintf(pItem->szGrade,"%s","Dev");
		else if(pCharacter->IsGrade251()) 
				sprintf(pItem->szGrade,"%s","Admin");
		else if(pCharacter->IsGrade250()) 
				sprintf(pItem->szGrade,"%s","FMod");
		else
				sprintf(pItem->szGrade,"");
		...
		if ( ZGetGameTypeManager()->IsQuestDerived( ZGetGame()->GetMatch()->GetMatchType()) || (ZGetGame()->GetMatch()->GetMatchType() == MMATCH_GAMETYPE_QUEST_CHALLENGE) )
		...
		pItem->nDamageGekregen = pCharacter->GetStatus().Ref().nDamageGekregen;
		pItem->nDamageGemaakt = pCharacter->GetStatus().Ref().nDamageGemaakt;
...
void ZCombatInterface::GetResultInfo( void)
{
	...
	{
		// 초기 UI 설정
		strcpy( szFileName, "common/interface/loadable/rstbg_quest.jpg");
		pWidget = ZApplication::GetGameInterface()->GetIDLResource()->FindWidget( "QuestResult");
		if ( pWidget)
			pWidget->Show( true);
	...
	{
		// 초기 UI 설정
		strcpy( szFileName, "common/interface/loadable/rstbg_survival.jpg");
		pWidget = ZApplication::GetGameInterface()->GetIDLResource()->FindWidget( "SurvivalResult");
		if ( pWidget)
			pWidget->Show( true);
	...
	{
		// 초기 UI 설정
		strcpy( szFileName, "common/interface/loadable/rstbg_clan.jpg");
		pWidget = ZApplication::GetGameInterface()->GetIDLResource()->FindWidget( "ClanResult");
		if ( pWidget)
			pWidget->Show( true);
	...
	{
		strcpy( szFileName, "common/Interface/loadable/rstbg_deathmatch.jpg");
		pWidget = ZApplication::GetGameInterface()->GetIDLResource()->FindWidget( "DuelTournamentResult");
		if ( pWidget)
			pWidget->Show( true);
	...
	{
		// 초기 UI 설정
		if ( (ZGetLocale()->GetCountry() == MC_US) || (ZGetLocale()->GetCountry() == MC_BRAZIL) || (ZGetLocale()->GetCountry() == MC_INDIA) || (ZGetLocale()->GetCountry() == MC_BETA))
		{
