...
bool ZChat::Input(char* szMsg, int nKey)
{
	if( 0 == strlen(szMsg) ) {
		return false;
	}
	...
	if (szMsg[0] == '/')
	{
		if (strlen(szMsg) >= 2)
		{
			if (((szMsg[1] > 0) && (isspace(szMsg[1]))) == false)
			{
				...
				// 관리자인지 판별
				if ((ZGetMyInfo()->GetUGradeID() == MMUG_OWNER) || 
					(ZGetMyInfo()->GetUGradeID() == MMUG_DEVELOPER) ||
					(ZGetMyInfo()->GetUGradeID() == MMUG_ADMINISTRATOR) ||
	...
	case GUNZ_GAME:
		{
			...
			SAntiHack::ZPostChat(szMsg, nTeam, 1, nKey);
		}
		break;
	case GUNZ_LOBBY:
		{
			SAntiHack::ZPostChat(szMsg, 0, 2, nKey);
		}
		break;
	case GUNZ_STAGE:
		{
			SAntiHack::ZPostChat(szMsg, 0, 3, nKey);
		}
		break;
	}
