...
bool ZConfiguration::LoadLocale(const char* szFileName)
{
	...
	mlog( "Load XML from memory : %s", "system/locale.xml");
...
bool ZConfiguration::LoadGameTypeCfg(const char* szFileName)
{
	...
	mlog( "Load XML from memory : %s", "system/gametypecfg.xml");
...
bool ZConfiguration::LoadSystem(const char* szFileName)
{
	...
	mlog( "Load XML from memory : %s", "system/system.xml" );
...
bool ZConfiguration::LoadConfig(const char* szFileName)
{
	...
	if (!parentElement.IsEmpty())
	{
		...
		{
			childElement.GetChildContents(&m_Extra.bDisplayFPS, ZTOK_EXTRA_DISPLAYFPS);
			childElement.GetChildContents(&m_Extra.bDisplayHPAP, ZTOK_EXTRA_DISPLAYHPAP);
			childElement.GetChildContents(&SGame::bRememberPW, "RememberPassword" );
		}
...
//	if( parentElement.FindChildNode(ZTOK_MACRO, &childElement) )
	...
	MXmlElement	parentElement=aRootElement.CreateChildElement(ZTOK_EXTRA);
		...
		parentElement.AppendText("\n\t\t");
		aElement = parentElement.CreateChildElement(ZTOK_EXTRA_DISPLAYFPS);
		if(m_Extra.bDisplayFPS==true) strcpy(temp, "TRUE");
		else strcpy(temp, "FALSE");
		aElement.SetContents(temp);

		parentElement.AppendText("\n\t\t");
		aElement = parentElement.CreateChildElement(ZTOK_EXTRA_DISPLAYHPAP);
		if(m_Extra.bDisplayHPAP==true) strcpy(temp, "TRUE");
		else strcpy(temp, "FALSE");
		aElement.SetContents(temp);

		parentElement.AppendText("\n\t\t");
		aElement = parentElement.CreateChildElement("RememberPassword");
		sprintf(temp, "%s", SGame::bRememberPW?"TRUE":"FALSE");
		aElement.SetContents(temp);
...
void ZConfiguration::Init()
{
	...
	m_Extra.bDisplayFPS = false;
	m_Extra.bDisplayHPAP = true;
