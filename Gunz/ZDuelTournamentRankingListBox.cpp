...
void ZDuelTournamentRankingListBox::LoadInterfaceImgs()
{
	if (m_pBmpRankingItemBg == NULL) {
		...
		((MBitmapR2*)m_pBmpRankingItemBg)->Create( "DuelTournamentRankingItemBg.png", RGetDevice(), "common/Interface/loadable/DuelTournamentRankingItemBg.png");
	}
	if (m_pBmpArrowUp == NULL) {
		m_pBmpArrowUp = new MBitmapR2;
		((MBitmapR2*)m_pBmpArrowUp)->Create( "arrow_up.tga", RGetDevice(), "common/Interface/loadable/arrow_up.tga");
	}
	if (m_pBmpArrowDown == NULL) {
		m_pBmpArrowDown = new MBitmapR2;
		((MBitmapR2*)m_pBmpArrowDown)->Create( "arrow_down.tga", RGetDevice(), "common/Interface/loadable/arrow_down.tga");
	}
	if (m_pBmpArrowBar == NULL) {
		m_pBmpArrowBar = new MBitmapR2;
		((MBitmapR2*)m_pBmpArrowBar)->Create( "arrow_bar.tga", RGetDevice(), "common/Interface/loadable/arrow_bar.tga");
	}
}
