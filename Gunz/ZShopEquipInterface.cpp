...
#ifndef _DEBUG
#if defined(LOCALE_BRAZIL) || defined(LOCALE_BETA) || defined(LOCALE_INDIA) || defined(LOCALE_US) || defined(LOCALE_JAPAN) || defined(LOCALE_KOREA) || defined(LOCALE_NHNUSA)
	{
		if ( nTabIndex == 2)
			return;
	}
...
//  Bounty Shower In Game
	...
	szColor = (nReqBounty > ZGetMyInfo()->GetVipCoins()) ? szRed : szGray;
	sprintf(sz2, "%s%d", szColor, ZGetMyInfo()->GetVipCoins());
	strcat(sz1, sz2);
	pTextArea[2]->AddText(sz1);
}
