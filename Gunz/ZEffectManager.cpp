...
bool ZEffectManager::Create(void)
{
	...
	for(int i=0; i<MUZZLESMOKE_COUNT; i++){
		sprintf(szFileName, "common/sound_sfx/muzzle_smoke0%d.tga", i+1);
		m_pEBSMuzzleSmoke[i]  = new ZEffectBillboardSource(szFileName);
	}

	for(int i=0; i<MUZZLESMOKE_SHOTGUN_COUNT; i++){
		sprintf(szFileName, "common/sound_sfx/muzzle_smoke4%d.tga", i+1);
		m_pEBSMuzzleSmokeShotgun[i]  = new ZEffectBillboardSource(szFileName);
	}

	for(int i=0; i<SMOKE_COUNT; i++){
		sprintf(szFileName, "common/sound_sfx/smoke0%d.tga", i+1);
		m_pEBSSmokes[i] = new ZEffectBillboardSource(szFileName);
	}
	for(int i=0; i<BLOOD_COUNT; i++){
		sprintf(szFileName, "common/sound_sfx/blood0%d.tga", i+1);
		m_pEBSBloods[i] = new ZEffectBillboardSource(szFileName);
	}
	for(int i=0; i<BLOODMARK_COUNT; i++){
		sprintf(szFileName, "common/sound_sfx/blood-mark0%d.tga", i+1);
		m_pEBSBloodMark[i] = new ZEffectBillboardSource(szFileName);
	}

	m_pEBSLightTracer = new ZEffectBillboardSource("common/sound_sfx//gz_sfx_tracer.bmp");
	
//	m_pEBSBulletMark[0] = new ZEffectBillboardSource("common/sound_sfx/bullet-mark01.tga");
//	m_pEBSBulletMark[1] = new ZEffectBillboardSource("common/sound_sfx/bullet-mark02.tga");
	m_pEBSBulletMark[0] = new ZEffectBillboardSource("common/sound_sfx/gz_sfx_shotgun_bulletmark01.tga");
	m_pEBSBulletMark[1] = new ZEffectBillboardSource("common/sound_sfx/gz_sfx_shotgun_bulletmark02.tga");
	m_pEBSBulletMark[2] = new ZEffectBillboardSource("common/sound_sfx/gz_effect004.tga");

	for(int i=0; i<RIFLEFIRE_COUNT; i++){
		sprintf(szFileName, "common/sound_sfx/gz_sfx_mf0%d.bmp", i+1);
		m_pEBSRifleFire[i][0] = new ZEffectBillboardSource(szFileName);
		sprintf(szFileName, "common/sound_sfx/gz_sfx_mf1%d.bmp", i+1);
		m_pEBSRifleFire[i][1] = new ZEffectBillboardSource(szFileName);
	}

	for(int i=0; i<SHOTGUNFIRE_COUNT; i++) {
		sprintf(szFileName, "common/sound_sfx/gz_sfx_mf4%d.bmp", i+1);
		m_pEBSShotGunFire[i] = new ZEffectBillboardSource(szFileName);
	}

	for(int i=0; i<GUNFIRE_COUNT; i++){
		sprintf(szFileName, "common/sound_sfx/gz_sfx_mf2%d.bmp", i+1);
		m_pEBSGunFire[i][0] = new ZEffectBillboardSource(szFileName);
		sprintf(szFileName, "common/sound_sfx/gz_sfx_mf3%d.bmp", i+1);
		m_pEBSGunFire[i][1] = new ZEffectBillboardSource(szFileName);
	}

	m_pEffectMeshMgr = new RMeshMgr;

	if(m_pEffectMeshMgr->LoadXmlList("common/sound_sfx/effect_list.xml")==-1) {
	...
	m_pSwordEnchantEffect[4]	= m_pEffectMeshMgr->Get("ef_sworddam_gold");
...
//	m_pRocketSmokeEffect		= m_pEffectMeshMgr->Get("rocket_smoke_effect");
	...
	m_pSwordGold		= m_pEffectMeshMgr->Get("ef_sword_gold");
	...
	m_pEBSRing[0] = new ZEffectBillboardSource("common/sound_sfx/gd_effect_001.tga");
	m_pEBSRing[1] = new ZEffectBillboardSource("common/sound_sfx/gd_effect_002.tga");

	m_pEBSDash[0] = new ZEffectBillboardSource("common/sound_sfx/gz_effect_dash01.tga");
	m_pEBSDash[1] = new ZEffectBillboardSource("common/sound_sfx/gz_effect_dash02.tga");

	m_pEBSLanding = new ZEffectBillboardSource("common/sound_sfx/ef_gz_footstep.tga");

	m_pEBSWaterSplash	= new ZEffectBillboardSource("common/sound_sfx/gd_effect_006.tga");
	m_pWaterSplash	= m_pEffectMeshMgr->Get("water_splash");

	m_pEBSWorldItemEaten	= new ZEffectBillboardSource("common/sound_sfx/ef_sw.bmp");
	...
	// Black Ops Mode
	m_pLeaderRedEffect = m_pEffectMeshMgr->Get("ef_LeaderRed");
    m_pLeaderBlueEffect = m_pEffectMeshMgr->Get("ef_LeaderBlue"); 
	...
	m_BulletMarkList.Create("common/sound_sfx/gz_sfx_shotgun_bulletmark01.tga");
	m_LightFragments.Create("common/sound_sfx/gz_sfx_tracer.bmp");
	m_LightFragments.SetScale(rvector(4.0f,0.8f,1.0f));

	m_BillboardLists[0].Create("common/sound_sfx/muzzle_smoke01.tga");
	m_BillboardLists[1].Create("common/sound_sfx/muzzle_smoke02.tga");
	m_BillboardLists[2].Create("common/sound_sfx/muzzle_smoke03.tga");
	m_BillboardLists[3].Create("common/sound_sfx/muzzle_smoke04.tga");

	m_BillboardLists[4].Create("common/sound_sfx/smoke_rocket.tga");

	m_ShadowList.Create("common/sound_sfx/gz_shadow.tga");

	m_BillBoardTexAniList[0].Create("common/sound_sfx/gd_effect_020.bmp");
	m_BillBoardTexAniList[1].Create("common/sound_sfx/gd_effect_021.bmp");
	m_BillBoardTexAniList[2].Create("common/sound_sfx/gd_effect_019.bmp");
	m_BillBoardTexAniList[3].Create("common/sound_sfx/ef_magicmissile.bmp");
	m_BillBoardTexAniList[4].Create("common/sound_sfx/ef_methor_smoke.tga");
...
*/
	...
	m_pWeaponEnchant[ZC_ENCHANT_GOLD]		= new ZEffectWeaponEnchant( m_pSwordGold  ,veczero,veczero, NULL );
...
void ZEffectManager::AddTrackGold(rvector &pos)
{
	int Add = rand() % 20;
	float fStartSize = 10 + Add;
	float fEndSize = 20 + Add;
...
*/
void ZEffectManager::AddEnchantGold2(ZObject* pObj)
{
	if(pObj==NULL) return;
...
void ZEffectManager::AddSwordEnchantEffect(ZC_ENCHANT type,const rvector& Target,DWORD start_time, float fScale)
{
	...
	RMesh* pMesh = NULL;
		...
		if( type ==  ZC_ENCHANT_FIRE )
		{
			pMesh = m_pSwordEnchantEffect[0];
		}
		else if( type ==  ZC_ENCHANT_COLD )
		{
			pMesh = m_pSwordEnchantEffect[1];
		}
		else if( type ==  ZC_ENCHANT_LIGHTNING )
		{
			pMesh = m_pSwordEnchantEffect[2];
		}
		else if( type ==  ZC_ENCHANT_POISON )
		{
			pMesh = m_pSwordEnchantEffect[3];
		}
		else if( type ==  ZC_ENCHANT_GOLD )
		{
			pMesh = m_pSwordEnchantEffect[4];
		}
...
void ZEffectManager::AddLighteningEffect(rvector& Target)
{
	...
	RMesh* pMesh = m_pEffectMeshMgr->Get("ef_lightening");
...
void ZEffectManager::AddPoisonEffect(rvector& Target,int nCnt)
{
	ZEffect* pNew = NULL;

	rvector dir = rvector(0.f,1.f,0.f);

	RMesh* pMesh = m_pEffectMeshMgr->Get("ef_poisonstorm");
...
// Black Ops Mode
void ZEffectManager::AddLeaderRedIcon(ZObject *pObj)
{
	ZEffect* pNew = NULL;
	pNew = new ZEffectIconLoopStar(m_pLeaderRedEffect,pObj);
	((ZEffectIconLoop*)pNew)->SetAlignType(1);
	((ZEffectIconLoop*)pNew)->m_type = eq_parts_pos_info_Spine2;
	Add(pNew);
}

void ZEffectManager::AddLeaderBlueIcon(ZObject *pObj)
{
	ZEffect* pNew = NULL;
	pNew = new ZEffectIconLoopStar(m_pLeaderBlueEffect,pObj);
	((ZEffectIconLoop*)pNew)->SetAlignType(1);
	((ZEffectIconLoop*)pNew)->m_type = eq_parts_pos_info_Spine2;
	Add(pNew);
}
