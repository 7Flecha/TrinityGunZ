...
string GetItemSpecString(MMatchItemDesc* pItemDesc)
{
	...
	if ( pItemDesc->IsEnchantItem())
	{
		...
		case MWT_ENCHANT_GOLD :
			sprintf( temp, "<%s>\n", ZMsg( MSG_WORD_ATTRIBUTE_GOLD));
			str += temp;
			str += DelayTemp;
			if ( pItemDesc->m_nDamage.Ref())
			{
...
//#ifndef _PUBLISH
		...
		if (bSpendable)	// 구입 갯수 입력 가능한 아이템
		{
			...
			OpenCountableConfirmDlg( m_pItem->GetDesc()->m_pMItemName->Ref().m_szItemName, m_pItem->GetIcon(), m_pItem->GetDesc()->m_nBountyPrice.Ref(), nMaxBuyCount, this);
		}
		else	// 하나씩 구입하는 아이템
		{
			OpenSimpleConfirmDlg(this);
