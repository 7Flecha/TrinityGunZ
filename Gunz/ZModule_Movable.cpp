...
{
	int nummer = 0;
	if(sscanf(ZGetGameClient()->GetStageName(), "!g%i", &nummer))
	{
		...
		{
			float gravity_mod = (nummer * 10.0f);
			m_Velocity.z = max( m_Velocity.z - (GRAVITY_CONSTANT*0.1f*gravity_mod)*fDelta,-MAX_FALL_SPEED);
		}
	}
	else
	{
