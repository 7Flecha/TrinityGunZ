...
#define STAGESETTING_TRAINING_DEFAULT	1
static struct _STAGESETTING_TRAINING
{
	bool	Value;
	char	szText[32];
} StageSetting_Training[STAGESETTING_TRAINING_MAX] = 
{ {true, "ON"}, {false, "OFF"} };

/////////////////////////////////
#define STAGESETTING_NOLEAD_MAX		2
#define STAGESETTING_NOLEAD_DEFAULT	0
static struct _STAGESETTING_NOLEAD
{
	bool	Value;
	char	szText[32];
} StageSetting_NoLead[STAGESETTING_NOLEAD_MAX] = 
{ {true, "ON"}, {false, "OFF"} };
...
/////////////////////////////////////////////////////////////////////////////////////////////
	...
	BUILD_STAGESETTING_ITEM("StageTraining", pOutNode->bIsTraining,
							StageSetting_Training, STAGESETTING_TRAINING_MAX);

	BUILD_STAGESETTING_ITEM("StageNoLead", pOutNode->bIsNoLead,
							StageSetting_NoLead, STAGESETTING_NOLEAD_MAX);
...
/////////////////////////////////////////////////////////////////////////////////////////
	...
	SHOWSTAGESETTING_ITEM("StageTraining", pStageSetting->bIsTraining,
		STAGESETTING_TRAINING_MAX, StageSetting_Training);

	SHOWSTAGESETTING_ITEM("StageNoLead", pStageSetting->bIsNoLead,
		STAGESETTING_NOLEAD_MAX, StageSetting_NoLead);
...
#define GET_LIMITLEVEL(_value)		sprintf(szText,"%s %d",ZMsg(MSG_WORD_LEVELDIFF),_value);
	...
	// 게임중참가
	strcpy( StageSetting_Training[0].szText, ZMsg(MSG_WORD_ON));
	strcpy( StageSetting_Training[1].szText, ZMsg(MSG_WORD_OFF));
	INITSTAGESETTING_ITEM("StageTraining", pStageSetting->bIsTraining,
		STAGESETTING_TRAINING_MAX, StageSetting_Training, STAGESETTING_TRAINING_DEFAULT);

	// 게임중참가
	strcpy( StageSetting_NoLead[0].szText, ZMsg(MSG_WORD_ON));
	strcpy( StageSetting_NoLead[1].szText, ZMsg(MSG_WORD_OFF));
	INITSTAGESETTING_ITEM("StageNoLead", pStageSetting->bIsNoLead,
		STAGESETTING_NOLEAD_MAX, StageSetting_NoLead, STAGESETTING_NOLEAD_DEFAULT);
...
//		if ((ZGetGameClient()) && (ZGetGameClient()->GetServerMode() == MSM_CLAN) || (ZGetGameClient()->GetServerMode() == MSM_CLAN))
//		{
...
/////////////////////////////////////////////////////////////////////////////////
	...
	if ( pCBType && (pCBType->GetCount() > 0))
	{
		...
		if ( strcmp( pCBType->GetString( pCBType->GetSelIndex()), ZMsg(MSG_MT_DEATHMATCH_SOLO)) == 0)
			...
			nGameType = 14;
		else if ( strcmp( pCBType->GetString( pCBType->GetSelIndex()), "Team Training") == 0)
			nGameType = 15;
