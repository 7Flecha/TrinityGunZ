...
void ZOptionInterface::InitInterfaceOption(void)
{
	...
	*/
	...
	//	ComboBox
	{
		...
		if(pWidget)	{
			if(ZGetConfiguration()->GetVideo()->nMapTexLevel == 8) {
				pWidget->SetSelIndex(3);
			} else if(ZGetConfiguration()->GetVideo()->nMapTexLevel == 12) {
		...
		if(pWidget)	{
			if(ZGetConfiguration()->GetVideo()->nEffectLevel > 3) {
				pWidget->SetSelIndex(3);
			} else {
	...
	//	Button
	{
		...
		pWidget = (MButton*)pResource->FindWidget("Shader"); //SHADERISHERE
		if( pWidget )
		{
		...
		/*pWidget = (MButton*)pResource->FindWidget("Archetype"); //Archetype
		if( pWidget )
		{
	...
	// Extra
	{	
		MButton* pDisplayFPS = (MButton*)pResource->FindWidget("DisplayFPS");
		if (pDisplayFPS)
		{
			pDisplayFPS->SetCheck(Z_EXTRA_DISPLAYFPS);
		}

		MButton* pDisplayHPAP = (MButton*)pResource->FindWidget("DisplayHPAP");
		if (pDisplayHPAP)
		{
			pDisplayHPAP->SetCheck(Z_EXTRA_DISPLAYHPAP);
		}
	}
	...
	*/

	{
		...
		if(pWidget)	{
			...
			ZGetGame()->GetWorld()->GetBsp()->SetWhiteMap(false);
			...
			if(pWidget->GetSelIndex() == 3) {
				...
				TexLevel = 4;
				if(ZGetGame()) {
					ZGetGame()->GetWorld()->GetBsp()->SetWhiteMap(true);
...
/*		DWORD flag = 0;
		...
		if(pWidget)
		{
			MButton* pWidgetShader = (MButton*)pResource->FindWidget("Shader");
			MButton* pWidgetDynamicLight = (MButton*)pResource->FindWidget("DynamicLight");
			MButton* pWidgetLightMap = (MButton*)pResource->FindWidget("LightMap");
			MButton* pWidgetReflection = (MButton*)pResource->FindWidget("Reflection");

			MComboBox* pWidgetScreenResolution = (MComboBox*)pResource->FindWidget("ScreenResolution");

			Z_VIDEO_ARCHETYPE = pWidget->GetCheck();

			if( Z_VIDEO_ARCHETYPE )
			{

			if( ZGetConfiguration()->GetVideo()->nColorBits != 8) {
				ZGetConfiguration()->GetVideo()->nColorBits = 8;
			}


			if( ZGetConfiguration()->GetVideo()->nMapTexLevel != 8 ) {
				ZGetConfiguration()->GetVideo()->nMapTexLevel = 8;
				SetMapTextureLevel(8);
				//SetObjectTextureLevel(8);
				//flag |= RTextureType_Object;
				flag |= RTextureType_Map;
			}
			...
			if( ZGetConfiguration()->GetVideo()->nMapTexLevel == 8 ) {
				ZGetConfiguration()->GetVideo()->nMapTexLevel = 2;
				SetMapTextureLevel(2);
				//SetObjectTextureLevel(2);
				//flag |= RTextureType_Object;
				flag |= RTextureType_Map;
			}
	...
	//Extra
	{
		...
		if(pDisplayFPS)
		{
			if (Z_EXTRA_DISPLAYFPS != pDisplayFPS->GetCheck()) 
			{
				Z_EXTRA_DISPLAYFPS = pDisplayFPS->GetCheck();
				if (Z_EXTRA_DISPLAYFPS)
					ZGetConfiguration()->GetExtra()->bDisplayFPS = true;
				else
					ZGetConfiguration()->GetExtra()->bDisplayFPS = false;
			}
		}
		...
		if(pDisplayHPAP)
		{
			if (Z_EXTRA_DISPLAYHPAP != pDisplayHPAP->GetCheck()) 
			{
				Z_EXTRA_DISPLAYHPAP = pDisplayHPAP->GetCheck();
				if (Z_EXTRA_DISPLAYHPAP)
					ZGetConfiguration()->GetExtra()->bDisplayHPAP = true;
				else
					ZGetConfiguration()->GetExtra()->bDisplayHPAP = false;
			}
		}
	}
	{
...
void ZOptionInterface::AdjustMultipliedWidgetsManually()
{
	...
	if (pWidget) {
		...
		//NICK: NEW LOGIN
		pWidget = pResource->FindWidget( "NetmarbleLoginFrame");
...
BEGIN_IMPLEMENT_LISTENER(ZGetRegisterFrameListener, MBTN_CLK_MSG)
	ZGetOptionInterface()->InitInterfaceOption();
	ZIDLResource* pResource = ZApplication::GetGameInterface()->GetIDLResource();
	MWidget* pWidget = pResource->FindWidget("Register");
	pWidget->Show(true, true);
	SGame::bRegisterPage = true;

END_IMPLEMENT_LISTENER()

BEGIN_IMPLEMENT_LISTENER(ZGetRegisterOKListener, MBTN_CLK_MSG)

END_IMPLEMENT_LISTENER()

BEGIN_IMPLEMENT_LISTENER(ZGetRegisterCANCELListener, MBTN_CLK_MSG)
	ZGetOptionInterface()->InitInterfaceOption();
	ZIDLResource* pResource = ZApplication::GetGameInterface()->GetIDLResource();
	MWidget* pWidget = pResource->FindWidget("Register");
	pWidget->Show(false, false);
	SGame::bRegisterPage = false;
END_IMPLEMENT_LISTENER()
