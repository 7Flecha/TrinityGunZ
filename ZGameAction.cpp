...
bool ZGameAction::OnPeerSkill(MCommand* pCommand)
{
	...
	switch(nSkill)	
	{
		...
		case ZC_SKILL_UPPERCUT: 
			{
				//SeaTroll Anti Flip Hack
				SAntiHack::AddFlip(pCommand->GetSenderUID());
				// Normal Flip Post
		...
		case ZC_SKILL_SPLASHSHOT: 
			{
				//SeaTroll Anti Massive Hack
				SAntiHack::AddMassive(pCommand->GetSenderUID());
				// Normal Massive post
...
bool ZGameAction::OnEnchantDamage(MCommand* pCommand)
{
	...
	bool bQuestGame = ZGetGame()->GetMatch()->IsQuestDrived();
	bool bCQuestGame = ZGetGameClient()->GetMatchStageSetting()->GetGameType() == MMATCH_GAMETYPE_QUEST_CHALLENGE;
	...
	if(pDesc)
	{
		switch(pOwnerCharacter->GetEnchantType())
		{
			case ZC_ENCHANT_FIRE :
				...
				if((bQuestGame) || (bCQuestGame)) return false;
				if(pOwnerCharacter)
				{
					ZCharacter* ptargetCharacter =(ZCharacter*) ZGetCharacterManager()->Find(targetUID);
					rvector post = ptargetCharacter->GetPosition();
					ZGetEffectManager()->AddMethorEffect((rvector(post.x,post.y,post.z)) , 1 );
					ZGetEffectManager()->AddSwordEnchantEffect(ZC_ENCHANT_FIRE,pos,0, 0.2f);
				}
				else
				{
					ZGetEffectManager()->AddMethorEffect((rvector(pos.x,pos.y,pos.z)) , 1 );
				}
				break;
			case ZC_ENCHANT_COLD :
				...
				if(pOwnerCharacter)
				{
					ZCharacter* ptargetCharacter =(ZCharacter*) ZGetCharacterManager()->Find(targetUID);
					rvector post = ptargetCharacter->GetPosition();
					ZGetEffectManager()->AddBlizzardEffect((rvector(post.x,post.y,post.z)) , 1 );
					ZGetEffectManager()->AddSwordEnchantEffect(ZC_ENCHANT_COLD,pos,0, 0.2f);
				}
				else
				{
					ZGetEffectManager()->AddBlizzardEffect((rvector(pos.x,pos.y,pos.z)) , 1 );
				}
				break;
			case ZC_ENCHANT_POISON :
				...
				if(pOwnerCharacter)
				{
					ZCharacter* ptargetCharacter =(ZCharacter*) ZGetCharacterManager()->Find(targetUID);
					rvector post = ptargetCharacter->GetPosition();
					ZGetEffectManager()->AddPoisonEffect((rvector(post.x,post.y,post.z)) , 1 );
					ZGetEffectManager()->AddSwordEnchantEffect(ZC_ENCHANT_POISON,pos,0, 0.2f);
				}
				else
				{
					ZGetEffectManager()->AddPoisonEffect((rvector(pos.x,pos.y,pos.z)) , 1 );
				}
			...
			case ZC_ENCHANT_LIGHTNING : 
				...
				if(pOwnerCharacter)
				{
					ZCharacter* ptargetCharacter =(ZCharacter*) ZGetCharacterManager()->Find(targetUID);
					rvector post = ptargetCharacter->GetPosition();
					ZGetEffectManager()->AddLighteningEffect(rvector(post.x,post.y,post.z));
					ZGetEffectManager()->AddSwordEnchantEffect(ZC_ENCHANT_LIGHTNING,pos,0, 0.2f);
				}
				else
				{
					ZGetEffectManager()->AddLighteningEffect(rvector(pos.x,pos.y,pos.z));
				}
				ApplyLightningEnchantDamage(pTarget, pOwnerCharacter, pDesc->m_nDamage.Ref(), pDesc->m_nDelay.Ref());	
				break;
			case ZC_ENCHANT_GOLD :
				ApplyGoldEnchantDamage(pTarget, pOwnerCharacter, pDesc->m_nDamage.Ref(), pDesc->m_nDelay.Ref());	
				break;
		};
	}
	return true;
...
bool ZGameAction::ApplyGoldEnchantDamage(ZObject* pTarget, ZObject* pOwner, int nDamage, int nDuration)
{
	if (!pTarget) return false;

	ZModule_GoldDamage *pMod = (ZModule_GoldDamage*)pTarget->GetModule(ZMID_GOLDDAMAGE);
	if(!pMod) return false;
