///////////////////////////////////////////////////////////////////////////
//																		 //
//		  ZGame.cpp Edited by SeaTroll									 //
//		  Last Edit: 19-12-2012											 //
//																		 //
///////////////////////////////////////////////////////////////////////////
#include "stdafx.h"
...
#define SNOW_PARTICLE_COUNT_PER_SECOND		400		// ´ë·« ÃÊ´ç 300°³Âë ¿ø·¡ ÇÁ·¹ÀÓ´ç 14°³¿´À½.
	...
	void Create()
	{
		...
		// ÆÄÆ¼Å¬ »ý¼º
		m_pParticles[0] = RGetParticleSystem()->AddParticles("common/sound_sfx/water_splash.bmp", 25.0f);
		m_pParticles[1] = RGetParticleSystem()->AddParticles("common/sound_sfx/water_splash.bmp", 10.0f);
		m_pParticles[2] = RGetParticleSystem()->AddParticles("common/sound_sfx/water_splash.bmp", 5.0f);
	}
	void Destroy()
	{
		if (!IsSnowTownMap()) return;
...
//	if ((m_pMyCharacter->IsDie() == false) && (m_pMyCharacter->GetHP() <= 0))
	...
	if(ZGetGameTypeManager()->IsCQuestDerived(ZGetGameClient()->GetMatchStageSetting()->GetGameType()))
	{
		ZPostNewQuestGameKill(); //chellangequest
	}
...
void ZGame::OnPreDraw()
{
	...
	GetWorld()->GetBsp()->SetWireframeMode(ZGetSGame()->bWireFrame);
...
////////////////////////////////////////////////////////////////////
	...
	//{
	//	ZPostSkill( ZC_SKILL_UPPERCUT, MMCIP_MELEE);
	//}
	//if(GetAsyncKeyState( VK_LEFT ))
	//{
	//	rvector vPos = ZGetGame()->m_pMyCharacter->GetPosition();
	//	ZPostShotMelee( vPos, 1);
	//	ZPostChannelChat(ZGetGameClient()->GetPlayerUID(), ZGetGameClient()->GetChannelUID(), "Spam");
	//}
	//if(GetAsyncKeyState( VK_RIGHT ))
	//{
	//	ZPostStageChat(ZGetGameClient()->GetPlayerUID(), ZGetGameClient()->GetStageUID(), "Spam");
	//	ZPostPeerChat("spam",  MMT_ALL); SAVE
	//}
	///////////////////// 
...
bool ZGame::GetUserGradeIDColor(MMatchUserGradeID UGradeID,MCOLOR& UserNameColor,char* sp_name)
{
	switch (UGradeID) 
	{	
	case MMUG_OWNER:
	...
	case MMUG_DONATE3:
		{
			unsigned long R[] = { 255, 84, 255, 135, 0 }; 
			unsigned long G[] = { 48, 255, 228, 206, 0 }; 
			unsigned long B[] = { 255, 159, 181, 250, 0 }; 
			unsigned long A[] = { 255, 255, 255, 255, 255 }; 
			...
			if ((timeGetTime()-LastTime) <= 20)
			{
				while( true )
				{
					...
					{
						unsigned long Red = R[Count];
						unsigned long Green = G[Count];
						unsigned long Blue = B[Count];
						unsigned long Alpha = A[Count];
						UserNameColor = MCOLOR(Red,Green,Blue);
					}
					ZChatOutput(MCOLOR(ZCOLOR_CHAT_SYSTEM), "ALLOWAL" );
				}
	...
	case MC_PEER_NOLEAD_RANGE:
		{
			...
			{
				MTD_RangeInfo* pInfo = (MTD_RangeInfo*)MGetBlobArrayElement(pBlob, i);
				ZGetGameClient()->GetPeerPacketCrypter().Decrypt((char*)pInfo, sizeof(MTD_RangeInfo));

				if (m_pMyCharacter && ZGetGameClient()->GetPlayerUID() != pCommand->GetSenderUID())
				{
					ZCharacter* pCharacter = (ZCharacter*)ZGetCharacterManager()->Find(ZGetGameClient()->GetPlayerUID());
					rvector pos;
					pos.x = pInfo->fPosX;
					pos.y = pInfo->fPosY;
					pos.z = pInfo->fPosZ;

					if (pCharacter && pInfo->nLowId == ZGetGameClient()->GetPlayerUID().Low && !ZGetGame()->GetMatch()->IsTeamPlay() || ((m_pMyCharacter->IsTeam(pCharacter) && ZGetGame()->GetMatch()->GetTeamKillEnabled()) || !m_pMyCharacter->IsTeam(pCharacter)))
					{
						if (pCharacter == ZGetGame()->m_pMyCharacter && !pCharacter->IsNPC()) 
						{
							// Damage Save
							m_pMyCharacter->GetStatus().CheckCrc();
							m_pMyCharacter->GetStatus().Ref().nDamageGekregen += pInfo->fDamage;
							m_pMyCharacter->GetStatus().MakeCrc();

						}

						if(ZGetSGame()->bDamageCounter == true)
						{
							// HP AP
							int nHP = m_pMyCharacter->GetHP();
							int nAP = m_pMyCharacter->GetAP();
							...
							// HP AP AFTER DAMAGE
							nHP -= m_pMyCharacter->GetHP();
							nAP -= m_pMyCharacter->GetAP();

							// Damage Log
							char szPrint[256];
							sprintf(szPrint, "You Lost %i(HP) and %i(AP)!", nHP, nAP);
							ZChatOutput(MCOLOR(255,100,100),szPrint); 
						}
						else
						{
							// Get Damage
				...
				else
				{
					...
					pCharacter->GetStatus().Ref().nDamageGekregen += pInfo->fDamage;
					pCharacter->GetStatus().MakeCrc();
				}
			}
		}
		break;
	case MC_PEER_NOLEAD_MELEE:
		{
			MCommandParameter* pParam = pCommand->GetParameter(0);
			...
			for (int i = 0; i < nSize; ++i)
			{
				MTD_MeleeInfo* pInfo = (MTD_MeleeInfo*)MGetBlobArrayElement(pBlob, i);
				ZGetGameClient()->GetPeerPacketCrypter().Decrypt((char*)pInfo, sizeof(MTD_MeleeInfo));
				...
				if (m_pMyCharacter && ZGetGameClient()->GetPlayerUID() != pCommand->GetSenderUID())
				{
					...
					if (pCharacter && pInfo->nLowId == ZGetGameClient()->GetPlayerUID().Low && !ZGetGame()->GetMatch()->IsTeamPlay() || ((m_pMyCharacter->IsTeam(pCharacter) && ZGetGame()->GetMatch()->GetTeamKillEnabled()) || !m_pMyCharacter->IsTeam(pCharacter)))
					{
						...
						if(ZGetSGame()->bDamageCounter == true)
						{
							...
							// Get Damage
							m_pMyCharacter->OnDamaged(pCharacter, pos, ZD_MELEE, MWT_DAGGER, pInfo->fDamage, pInfo->fRatio);
							m_pMyCharacter->OnDamagedAnimation(pCharacter, SEM_WomanSlash5);
	...
	case ZC_WEAPON_SP_GRENADE : 
		{
			...
			for(int i = 0; i < 16; i++)
			{
				cItemDesc[i] = pDesc->m_szDesc[i];
			}
			char* pItemDesc = cItemDesc;
			pItemDesc = strlwr(pItemDesc);
			if(strstr(pItemDesc,"Molotov"))
				m_WeaponManager.AddMolotovGrenade(pos, velocity, pOwnerCharacter);
			else if(strstr(pItemDesc,"Ice"))
				m_WeaponManager.AddIceGrenade(pos, velocity, pOwnerCharacter);
			else if(strstr(pItemDesc,"Fire"))
				m_WeaponManager.AddFireGrenade(pos, velocity, pOwnerCharacter);
			else if(strstr(pItemDesc,"Poison"))
				m_WeaponManager.AddPoisonGrenade(pos, velocity, pOwnerCharacter);
			else if(strstr(pItemDesc,"Lightning"))
				m_WeaponManager.AddLightningGrenade(pos, velocity, pOwnerCharacter);
			else
...
void ZGame::OnExplosionSpecialGrenade(MUID uidOwner,rvector pos,float fDamage,float fRange,float fMinDamage,float fKnockBack,MMatchTeam nTeamID,const char* pSPGrenadeType)
{
	ZObject* pTarget = NULL;
...
//#define MIN_DMG			0.4f	// ÃÖ¼Ò ±âº» µ¥¹ÌÁö´Â ÀÌÁ¤µµ.
		...
		ZGameAction* pElementGrenade = NULL;
		if(pSPGrenadeType == "Ice")
		{
			pElementGrenade->ApplyColdEnchantDamage(pTarget, 40, 1500);
		}
		else if(pSPGrenadeType == "Fire")
		{
			pElementGrenade->ApplyFireEnchantDamage(pTarget, pOwnerCharacter, (fDamage*fDamageRange)/9, 1500);
		}
		else if(pSPGrenadeType == "Poison")
		{
			pElementGrenade->ApplyPoisonEnchantDamage(pTarget, pOwnerCharacter, (fDamage*fDamageRange)/10, 1500);
		}
		else if(pSPGrenadeType == "Lightning")
		{
			pElementGrenade->ApplyLightningEnchantDamage(pTarget, pOwnerCharacter, 2, 1000);
		}
		else
		{
			//Error
		}
		float fActualDamage = (fDamage * fDamageRange)/3;
		float fRatio = ZItem::GetPiercingRatio( MWT_FRAGMENTATION , eq_parts_chest );//¼ö·ùÅº°ú ·ÎÄÏ ±¸ºÐ¾ø´Ù..
		pTarget->OnDamaged(pOwnerCharacter,pos,ZD_EXPLOSION,MWT_FRAGMENTATION,fActualDamage,fRatio);
...
{
	//### ÀÌ ÇÔ¼ö¸¦ ¼öÁ¤ÇÏ¸é ¶È°°ÀÌ ¾Æ·¡ÀÇ InRanged()¿¡µµ Àû¿ëÇØ ÁÖ¾î¾ß ÇÕ´Ï´Ù. ###
...
void ZGame::OnPeerDie(MUID& uidVictim, MUID& uidAttacker)
{
	...
	if (pVictim == m_pMyCharacter)	
	{
		...
		else if (m_Match.IsWaitForRoundEnd())
		{
			if (m_CharacterManager.GetCount() > 2)
			{
...
//			sprintf(szMsg, "%s´ÔÀÌ ½º½º·Î ÆÐ¹èÇÏ¿´½À´Ï´Ù.", szAttacker);
	...
	else if (pAttacker == m_pMyCharacter)
	{
		...
		if(m_pMyCharacter->GetStatus().Ref().nKillStreakCount > 3 && m_pMyCharacter->GetStatus().Ref().nKillStreakCount < 11)
			ZPostKillStreak(szAttacker2, m_pMyCharacter->GetStatus().Ref().nKillStreakCount);

//		sprintf(szMsg, "´ç½ÅÀº %s´ÔÀ¸·ÎºÎÅÍ ½Â¸®ÇÏ¿´½À´Ï´Ù.", szVictim );
	...
	{
		//kill Streak
		if(m_pMyCharacter->GetStatus().Ref().nKillStreakCount > 3)
			ZPostKillStreak(szVictim2, 0, szAttacker2);
...
void ZGame::OnPeerSpawn(MUID& uid, rvector& pos, rvector& dir)
{
	...
	pCharacter->GetStatus().CheckCrc();
	pCharacter->GetStatus().Ref().nDamageGekregen = 0;
	pCharacter->GetStatus().Ref().nDamageGemaakt = 0;
	pCharacter->GetStatus().MakeCrc();
...
void ZGame::OnPeerDash(MCommand* pCommand)
{
	...
	int	code;
	pCommand->GetParameter(&code , 0, MPT_INT);
	if (code != SGame::GetPCode);
		return;
...
void ZGame::PostBasicInfo()
{
	...
	if ((int)(nNowTime - m_nLastTime[ZLASTTIME_BASICINFO]) >= nMoveTick)
	{
		...
		pbi.posz = (int(m_pMyCharacter->GetPosition().z * 0.1f)) * 10.f;*/	// ¿ÀÂ÷·Î ÀÎÇÑ ¹ö±×ÀçÇöÀ» ½±°Ô ÇÏ±EÀ§ÇØ 1ÀÇ ÀÚ¸®±ûÝEÀý»çÇÑ ¹öÀE
...
// Äù½ºÆ® Ä¿¸Çµåµé
		...
		// ·ê¿¡¼­µµ ÇÊÅÍ¸µÀÌ ¾ÈµÇ¸é ÀÌ°Ç delayed command°¡ ¾Æ´Ï´Ù
		default:
			{
				if (m_Match.GetRule())
				{
...
{
	// npc°¡ ¸Ê¿¡ ³¤ °æ¿ì ¼ø°£ÀÌµ¿½ÃÄÑ¼­¶óµµ ¹þ¾î³ª°Ô ÇÒ °ÍÀÎ°¡?
	// ÇöÀç´Â ¼­¹ÙÀÌ¹úÀÏ¶§¸¸À¸·Î Á¦ÇÑÇÑ´Ù (¿öÇÁ´Â ¸Å¿ì Á¶¾ÇÇÑ ÇØ°á¹ýÀÌ´Ù. ±âÁ¸ Äù½ºÆ®¿¡¼­ ¾ø´ø ¸÷¿öÇÁ°¡ ÀÏ¾î³ª À¯Àú ºÒ¸¸ÀÌ ÀÖÀ½)
