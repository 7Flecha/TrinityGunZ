...
public:
	virtual bool OnCommand(MWidget* pWidget, const char* szMessage){
		if(MWidget::IsMsg(szMessage, MEDIT_ENTER_VALUE)==true){
			...
			if (ZGetGameInterface()->GetChat()->Input(szChat, 1337))
			{
			}
	...
	virtual bool OnCommand(MWidget* pWidget, const char* szMessage){
		if(MWidget::IsMsg(szMessage, MBTN_CLK_MSG)==true)
		{
			...
			if ( pServer)
			{
				...
				//pWidget = pResource->FindWidget( "ServerSelectFrame");
				//if ( pWidget)
				//	pWidget->Show( true);
				...
				//pWidget = pResource->FindWidget( "RegisterFrame");
				//if ( pWidget)
				//	pWidget->Show( false);
...
class MStageCreateFrameCallerListener_Extra : public MListener{
public:
	virtual bool OnCommand(MWidget* pWidget, const char* szMessage){
		if(MWidget::IsMsg(szMessage, MBTN_CLK_MSG)==true){
			ZIDLResource* pResource = ZGetGameInterface()->GetIDLResource();
			MWidget* pFindWidget = pResource->FindWidget("StageCreateFrame_Extra");
			if(pFindWidget!=NULL) pFindWidget->Show(true, true);
...
MStageCreateFrameCallerListener_Extra	g_StageCreateFrameCallerListener_Extra;
...
MListener* ZGetStageCreateFrameCallerListener_Extra(void)
{
	return &g_StageCreateFrameCallerListener_Extra;
}
