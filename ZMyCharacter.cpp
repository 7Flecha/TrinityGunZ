...
//					m_nWallJump2Dir=1;
...
#endif
			{
				...
				int nummer = 0;
				if(sscanf(ZGetGameClient()->GetStageName(), "!j%i", &nummer))
				{
					...
					{
						float jump_force_mod = (nummer * 10.0f);
						rvector newVelocity = vel*0.5f + m_Accel.Ref()*fVel*.45f + rvector(0,0,JUMP_VELOCITY * 0.1f * jump_force_mod);
						SetVelocity(newVelocity);
					}
				}
				else
	...
	case ZDW_DASH :
		if(!strstr(ZGetGameClient()->GetStageName(),"[NF]"))
		{
			if(zStatus.m_bSkill) {	// 아직 상태를 벗어나지 않았다면,,
				...
				if(type == MWT_DAGGER || type == MWT_DUAL_DAGGER)
				{
...
{
	if(strstr(ZGetGameClient()->GetStageName(),"[NM]"))
	{
	// Notting
	}
	else
...
{
	if(strstr(ZGetGameClient()->GetStageName(),"[DMG3]"))
		ZCharacter::OnDamaged(pAttacker,srcPos,damageType,weaponType,(fDamage*3.f),fPiercingRatio,nMeleeType);
	else if(strstr(ZGetGameClient()->GetStageName(),"[DMG2]"))
		ZCharacter::OnDamaged(pAttacker,srcPos,damageType,weaponType,(fDamage*2.f),fPiercingRatio,nMeleeType);
	else
