...
protected:
	...
	//void OnMatchLoginRegister(MUID CommUID, const char* szUserID, const char* szPassword, int nCommandVersion, unsigned long nChecksumPack, char *szEncryptMd5Value, char* szName, char* szAge, char* szEmail);
...
protected:
	...
	bool ChannelAdd(const char* pszChannelName, const char* pszRuleName, MUID* pAllocUID, MCHANNEL_TYPE nType=MCHANNEL_TYPE_PRESET, int nMaxPlayers=DEFAULT_CHANNEL_MAXPLAYERS, int nLevelMin=-1, int nLevelMax=-1, const bool bIsEvent=false,
...
protected:
	...
	void OnAdminBan(const MUID& uidAdmin, const char* pszTargetName);
...
protected:
	...
	void OnNewQuestRequestDead(const MUID& uidVictim);
