		...
		case MC_MATCH_RESPONSE_LOGIN:
			{
				...
				pCommand->GetParameter(&nVipCoins,		6, MPT_INT);
...
//				unsigned char *szRandomValue = (unsigned char*)MGetBlobArrayElement(pBlob1, 0);
				...
				OnResponseMatchLogin(pCommand->GetSenderUID(), nResult, szServerName, MMatchServerMode(nServerMode), 
					szAccountID, MMatchUserGradeID(nUGradeID), MMatchPremiumGradeID(nPGradeID), nVipCoins, uidPlayer, bEnabledSurvivalMode, bEnabledDuelTournament, pbyGuidReqMsg);
			}
			break;
