...
void MMatchServer::OnStageCreate(const MUID& uidChar, char* pszStageName, bool bPrivate, char* pszStagePassword)
{
	...
	//Nick: Disable create room in event channel only admin enable
	if (pChannel->IsEvent() == true && !IsAdminGrade(pObj))
	{
		RouteResponseToListener(pObj, MC_MATCH_RESPONSE_STAGE_CREATE, MERR_CANNOT_STAFF_ONLY);
		return;
	}
...
void MMatchServer::OnPrivateStageJoin(const MUID& uidPlayer, const MUID& uidStage, char* pszPassword)
{
	...
	if (ugid == MMUG_DEVELOPER || ugid == MMUG_OWNER || ugid == MMUG_GAMEMASTER || ugid == MMUG_ADMINISTRATOR)
		bSkipPassword = true;
...
//		pObj->SetInvalidStageSettingDisconnectWaitInfo();
	...
	if (MGetGameTypeMgr()->IsQuestDerived( nLastGameType ) == true && MGetGameTypeMgr()->IsQuestDerived( pNode->nGameType ) == false)
		pNode->bForcedEntryEnabled = true;

	// TEST CQ
	if(MGetGameTypeMgr()->IsCQuestDerived( nLastGameType ) == true && MGetGameTypeMgr()->IsCQuestDerived( pNode->nGameType ) == false)
		pNode->bForcedEntryEnabled = true;
...
void MMatchServer::OnRequestStageSetting(const MUID& uidComm, const MUID& uidStage)
{
	...
	if (pChar && (MMUG_DEVELOPER == pChar->GetAccountInfo()->m_nUGrade)) 	
	{
					   ...
					   int* poutAttackerExp, int* poutVictimExp)
{
	...
	if (
		...
		(pStage->GetStageSetting()->IsTraining()) ||
		(pChannel->IsEvent())
		)
	{
		*poutAttackerExp = 0;
...
//		}
	...
	else if (nGameType == MMATCH_GAMETYPE_BOUNTY)
	{
		MMatchRuleBounty* pRuleBounty = (MMatchRuleBounty*)pStage->GetRule();

		if (pRuleBounty->GetBounty() == pAttacker->GetUID())
		{
			if (pAttacker != pVictim)
			{
				fGameExpRatio = fGameExpRatio * 1.5f;
			}
			else
			{
				fGameExpRatio = 0.0f;
	...
	if ((pVictim->GetAccountInfo()->m_nUGrade == MMUG_OWNER) || 
		(pVictim->GetAccountInfo()->m_nUGrade == MMUG_DEVELOPER) ||
		(pVictim->GetAccountInfo()->m_nUGrade == MMUG_GAMEMASTER))
	{
		nAttackerExp = nAttackerExp * 2;
	}
	// 죽인사람이 운영자, 개발자일 경우 경치다운 없음
	if ((!bSuicide) &&
		((pAttacker->GetAccountInfo()->m_nUGrade == MMUG_OWNER) || 
		(pAttacker->GetAccountInfo()->m_nUGrade == MMUG_DEVELOPER) ||
		(pAttacker->GetAccountInfo()->m_nUGrade == MMUG_GAMEMASTER)))
	{
		nVictimExp = 0;
	}
...
void MMatchServer::OnAdminBan(const MUID& uidAdmin, const char* pszTargetName)
{
    MMatchObject* pObj = GetObject(uidAdmin);
   if( 0 == pObj )
        return;
    ...
    if (m_MatchDBMgr.OnAdminBan(pTargetObj->GetAccountInfo()->m_nAID)) 
	{
        MMatchObjectCacheBuilder CacheBuilder;
        CacheBuilder.AddObject(pTargetObj);
        MCommand* pCmdCacheUpdate = CacheBuilder.GetResultCmd(MATCHCACHEMODE_REPLACE, this);
        ...
        MCommand* pCmdUIUpdate = CreateCommand(MC_ADMIN_BAN, MUID(0,0));
        pCmdUIUpdate->AddParameter(new MCommandParameterUID(pTargetObj->GetUID()));
        pCmdUIUpdate->AddParameter(new MCommandParameterBool(false));
        RouteToStage(pStage->GetUID(), pCmdUIUpdate);
    }
