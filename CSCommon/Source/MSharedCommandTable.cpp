...
void MAddSharedCommandTable(MCommandManager* pCommandManager, int nSharedType)
{
	...
	if(nSharedType&MSCT_MATCHSERVER || nSharedType&MSCT_CLIENT){
		...
		C(MC_MATCH_RESPONSE_LOGIN, "Match.ResponseLogin", "Response Login", MCDT_MACHINE2MACHINE)
			...
			P(MPT_BLOB,	"EncryptMsg")		// HShield¿¡¼­ »ç¿ëÇÏ´Â ¾ÏÈ£¸Þ¼¼Áö

			/* ServerKeeper¿¡¼­µµ »ç¿ëÀ» ÇÏ±âÀ§ÇØ¼­ ¿ÜºÎ·Î ¿Å±è. - by Ãß±³¼º.
		...
		C(MC_MATCH_CHANNEL_RESPONSE_JOIN, "Channel.ResponseJoin", "Response Join a Channel", MCDT_MACHINE2MACHINE)
			P(MPT_UID, "uidChannel")
			P(MPT_INT, "ChannelType")			// Á¢¼ÓÇÑ Ã¤³Î Å¸ÀÔ
			P(MPT_STR, "ChannelName")			// Á¢¼ÓÇÑ Ã¤³Î ÀÌ¸§
			P(MPT_BOOL, "EnableInterface");		// ·Îºñ ÀÎÅÍÆäÀÌ½º »ç¿ë ¿©ºÎ
		...
		C(MC_MATCH_CHANNEL_REQUEST_ALL_PLAYER_LIST, "Channel.RequestAllPlayerList", "Request Channel All Player List", MCDT_MACHINE2MACHINE)
			P(MPT_UID, "uidPlayer")
			P(MPT_UID, "uidChannel")
			P(MPT_UINT, "PlaceFilter")		// ·Îºñ, ´ë±â¹æ, °ÔÀÓÁß ÀÎ ÇÃ·¹ÀÌ¾îµé¿¡ ´ëÇÑ ÇÊÅÍ
			P(MPT_UINT, "Options")			// 0 - ÀÏ¹Ý, 1 - Å¬·£¿¡ °¡ÀÔµÇÁö ¾ÊÀº »ç¶÷, 2 - ³»Å¬·£¸¸
		...
		C(MC_MATCH_STAGE_LEAVE, "Stage.Leave", "Leave Stage", MCDT_MACHINE2MACHINE)
			P(MPT_UID, "uidPlayer")
			// P(MPT_UID, "uidStage") // ¼­¹ö¿¡ ÀÖ´Â µ¥ÀÌÅÍ¸¦ »ç¿ëÇÑ´Ù.
...
//			P(MPT_BLOB, "SKey")
		...
		C(MC_MATCH_RESPONSE_SELECT_CHAR, "Match.ResponseSelectChar", "Response Select Character", MCDT_MACHINE2MACHINE)
			P(MPT_INT, "Result")
			P(MPT_BLOB, "CharInfo")
			//¹öÇÁÁ¤º¸ÀÓ½ÃÁÖ¼® P(MPT_BLOB, "CharBuffInfo")
		...
		C(MC_MATCH_RESPONSE_CREATE_CHAR, "Match.ResponseCreateChar", "Response Create Character", MCDT_MACHINE2MACHINE)
			P(MPT_INT, "Result")		// °á°ú
			P(MPT_STR, "CharName")		// ¸¸µé¾îÁø Ä³¸¯ÅÍ ÀÌ¸§
		...
		C(MC_MATCH_RESPONSE_SHOP_ITEMLIST, "Match.ResponseShopItemList", "Response Shop Item List", MCDT_MACHINE2MACHINE)
			P(MPT_BLOB, "GambleItemList");		// À§Ä¡ ¹Ù²Þ - by SungE 2007-06-20
			P(MPT_BLOB, "ItemList");			
		...
		C(MC_MATCH_RESPONSE_CHARACTER_ITEMLIST, "Match.ResponseCharacterItemList", "Response Character Item List", MCDT_MACHINE2MACHINE)
			P(MPT_INT, "Bounty")
			P(MPT_INT, "VipCoins")
			P(MPT_BLOB, "EquipItemList")		// ÀåºñÇÏ°í ÀÖ´Â ¾ÆÀÌÅÛ
			P(MPT_BLOB, "ItemList");			// ÀåºñÇÑ ¾ÆÀÌÅÛ±îÁö Æ÷ÇÔÇÑ °®°íÀÖ´Â ¾ÆÀÌÅÛ
			P(MPT_BLOB, "GambleItemList")		// °×ºí ¾ÆÀÌÅÛ ¸ñ·Ï
			
		C(MC_MATCH_REQUEST_EQUIP_ITEM, "MatchRequestEquipItem", "Request Equip Item", MCDT_MACHINE2MACHINE)
			P(MPT_UID, "uidChar")
			P(MPT_UID, "uidItem")		// °¡Áø ¾ÆÀÌÅÛ UID
			P_MINMAX(MPT_UINT, "EquipmentSlot", 0, MMCIP_END-1)		// ÀåºñÇÏ°íÀÚ ÇÏ´Â ºÎÀ§
		...
		C(MC_MATCH_REQUEST_TAKEOFF_ITEM, "MatchRequestTakeoffItem", "Request Takeoff Item", MCDT_MACHINE2MACHINE)
			P(MPT_UID, "uid")
			P_MINMAX(MPT_UINT, "EquipmentSlot", 0, MMCIP_END-1)		// ¹þ°íÀÚ ÇÏ´Â ºÎÀ§
		...
		C(MC_MATCH_REMOVE_WORLDITEM, "Match.Request.Spawn.WorldItem", "Request Spawn WorldItem", MCDT_MACHINE2MACHINE)
			P(MPT_INT, "nWorldItemUID")		// world itemÀÇ UID
		...
		C(MC_MATCH_LADDER_REQUEST_CHALLENGE, "Match.Ladder.Request.Challenge", "Request Challenge a Ladder", MCDT_MACHINE2MACHINE)
			P(MPT_INT, "MemberCount")		// ·¹´õÆÀ ¸â¹ö ¼ö
			P(MPT_UINT, "Options")			// ¿É¼Ç
		...
		C(MC_MATCH_REQUEST_PROPOSAL, "Match.RequestProposal", "Request Proposal", MCDT_MACHINE2MACHINE)
			P(MPT_UID, "uidChar")			// ¿äÃ»ÇÑ Ä³¸¯ÅÍ UID
			P(MPT_INT, "ProposalMode")		// µ¿ÀÇ Á¾·ù - MMatchProposalModeÀÇ intÇü
			P(MPT_INT, "RequestID")			// ¿äÃ»ID - Å¬¶óÀÌ¾ðÆ®°¡ ÀÓÀÇ·Î °áÁ¤ÇÑ´Ù. ÀÚ½Å¸¸ ¾Ë°í ÀÖÀ¸¸é µÊ.
			P(MPT_INT, "ReplierCount")		// µ¿ÀÇ¸¦ ¹°¾îº¼ »ç¶÷¼ö
			P(MPT_BLOB, "ReplierCharNames")	// µ¿ÀÇÀÚ Ä³¸¯ÅÍ ÀÌ¸§ - AskCount¸¸Å­ÀÌ BLOB¿¡ µé¾îÀÖ´Ù.
		C(MC_MATCH_RESPONSE_PROPOSAL, "Match.ResponseProposal", "Response Proposal", MCDT_MACHINE2MACHINE)
			P(MPT_INT, "Result")			// °á°ú
			P(MPT_INT, "ProposalMode")		// µ¿ÀÇ Á¾·ù - MMatchAgreementModeÀÇ intÇü
			P(MPT_INT, "RequestID")			// ¿äÃ»ID
		C(MC_MATCH_ASK_AGREEMENT, "Match.AskAgreement", "Ask Agreement", MCDT_MACHINE2MACHINE)
			P(MPT_UID,	"uidProposer")		// Á¦¾ÈÀÚ Ä³¸¯ÅÍ UID
			P(MPT_BLOB,	"MembersCharName")	// Âü¿©ÀÚ Ä³¸¯ÅÍ ÀÌ¸§µé
			P(MPT_INT,	"ProposalMode")		// µ¿ÀÇ Á¾·ù - MMatchAgreementModeÀÇ intÇü
			P(MPT_INT,	"RequestID")		// ¿äÃ»ID
		C(MC_MATCH_REPLY_AGREEMENT, "Match.ReplyAgreement", "Reply Agreement", MCDT_MACHINE2MACHINE)
			P(MPT_UID,	"uidProposer")		// Á¦¾ÈÀÚ Ä³¸¯ÅÍ UID
			P(MPT_UID,	"uidChar")			// ´äº¯ÀÚ Ä³¸¯ÅÍ UID
			P(MPT_STR,	"Replier")			// ´äº¯ÀÚ Ä³¸¯ÅÍ ÀÌ¸§
			P(MPT_INT,	"ProposalMode")		// µ¿ÀÇ Á¾·ù
			P(MPT_INT,	"RequestID")		// ¿äÃ» ID
			P(MPT_BOOL,	"Agreement")		// µ¿ÀÇ ¿©ºÎ
		...
		C(MC_MATCH_CLAN_REQUEST_CREATE_CLAN, "Match.Clan.RequestCreateClan", "Request Create Clan", MCDT_MACHINE2MACHINE)
			P(MPT_UID, "uidChar")			// ¿äÃ»ÇÑ Ä³¸¯ÅÍ UID
			P(MPT_INT, "RequestID")			// ¿äÃ»ID - Å¬¶óÀÌ¾ðÆ®°¡ ÀÓÀÇ·Î °áÁ¤ÇÑ´Ù. ÀÚ½Å¸¸ ¾Ë°í ÀÖÀ¸¸é µÊ.
			P(MPT_STR, "ClanName")			// »ý¼ºÇÒ Å¬·£ ÀÌ¸§
			P(MPT_STR, "Member1CharName")	// ¹ß±âÀÎ Ä³¸¯ÅÍ ÀÌ¸§
			//P(MPT_STR, "Member2CharName")	// ¹ß±âÀÎ Ä³¸¯ÅÍ ÀÌ¸§
			//P(MPT_STR, "Member3CharName")	// ¹ß±âÀÎ Ä³¸¯ÅÍ ÀÌ¸§
			//P(MPT_STR, "Member4CharName")	// ¹ß±âÀÎ Ä³¸¯ÅÍ ÀÌ¸§
		...
		C(MC_MATCH_CLAN_ASK_SPONSOR_AGREEMENT, "Match.Clan.AskSponsorAgreement", "Ask Sponsor's Agreement", MCDT_MACHINE2MACHINE)
			P(MPT_INT, "RequestID")			// ¿äÃ»ID
			P(MPT_STR, "ClanName")
			P(MPT_UID, "uidClanMaster")		// ¸¶½ºÅÍ UID
			P(MPT_STR, "szClanMaster")		// ¸¶½ºÅÍ ÀÌ¸§
		C(MC_MATCH_CLAN_ANSWER_SPONSOR_AGREEMENT, "Match.Clan.AnswerSponsorAgreement", "Answer Sponsor's Agreement", MCDT_MACHINE2MACHINE)
			P(MPT_INT,	"RequestID")			// ¿äÃ»ID
			P(MPT_UID,	"uidClanMaster")		// ¸¶½ºÅÍ UID
			P(MPT_STR,	"SponsorCharName")		// ¹ß±âÀÎ Ä³¸¯ÅÍ ÀÌ¸§
			P(MPT_BOOL, "Answer")				// µ¿ÀÇ ¿©ºÎ
C(MC_MATCH_CLAN_REQUEST_AGREED_CREATE_CLAN, "Match.Clan.RequestAgreedCreateClan", "Request Agreed Create Clan", MCDT_MACHINE2MACHINE)
		...
		C(MC_MATCH_CLAN_REQUEST_JOIN_CLAN, "Match.Clan.RequestJoinClan", "Request Join Clan", MCDT_MACHINE2MACHINE)
			P(MPT_UID, "uidChar")		// ¿î¿µÀÚ uid
			P(MPT_STR, "ClanName")		// Å¬·£ ÀÌ¸§
			P(MPT_STR, "szJoiner")		// °¡ÀÔÀÚ Ä³¸¯ÅÍ ÀÌ¸§
		...
		C(MC_MATCH_CLAN_ASK_JOIN_AGREEMENT, "Match.Clan.AskJoinAgreement", "Ask Join Agreement", MCDT_MACHINE2MACHINE)
			P(MPT_STR, "ClanName")				// Å¬·£ ÀÌ¸§
			P(MPT_UID, "uidClanAdmin")			// ¿î¿µÀÚ UID
			P(MPT_STR, "szClanAdmin")			// ¿î¿µÀÚ Ä³¸¯ÅÍ ÀÌ¸§
		C(MC_MATCH_CLAN_ANSWER_JOIN_AGREEMENT, "Match.Clan.AnswerJoinAgreement", "Answer Join Agreement", MCDT_MACHINE2MACHINE)
			...
			P(MPT_STR,	"JoinerCharName")		// °¡ÀÔÀÚ Ä³¸¯ÅÍ ÀÌ¸§
			P(MPT_BOOL, "Answer")
		...
		C(MC_MATCH_CLAN_MASTER_REQUEST_CHANGE_GRADE, "Match.Clan.Master.RequestChangeGrade", "Request Change ClanGrade", MCDT_MACHINE2MACHINE)
			P(MPT_UID, "uidClanMaster")		// Å¬·£ ¸¶½ºÅÍ
			P(MPT_STR, "szMember")			// º¯°æÇÏ·Á°í ÇÏ´Â ¸â¹ö Ä³¸¯ÅÍÀÌ¸§
			P_MINMAX(MPT_INT, "Grade", 0, MCG_END-1)				// º¯°æÇÏ°íÀÚ ÇÏ´Â ±ÇÇÑ
		...
		C(MC_QUEST_NPC_SPAWN,	"Quest.NPCSpawn", "Npc Spawn", MCDT_MACHINE2MACHINE)
			P(MPT_UID,		"uidChar")				// NPC Á¶Á¾ÀÚ
			P(MPT_UID,		"nNpcUID")				// ÇÒ´çµÈ NPC ID
			P(MPT_UCHAR,	"nNpcType")				// NPC Á¾·ù
		...
		C(MC_QUEST_REQUEST_NPC_DEAD,	"Quest.Request.NPCDead", "Request Npc Dead", MCDT_MACHINE2MACHINE)
			P(MPT_UID, "uidChar")		// NPC Á¶Á¾ÀÚ
			P(MPT_UID, "uidNPC")		// ÇÒ´çµÈ NPC ID
			P(MPT_SVECTOR, "NpcPosition")	// NPC À§Ä¡
		C(MC_QUEST_NPC_DEAD,	"Quest.NPCDead", "Npc Dead", MCDT_MACHINE2MACHINE)
			P(MPT_UID, "uidKillerPlayer")	// Á×ÀÎ ÇÃ·¹ÀÌ¾î
			P(MPT_UID, "uidNPC")			// Á×´Â NPC id
		...
		C(MC_QUEST_COMBAT_STATE, "Quest.Combat.State", "Quest Combat State", MCDT_MACHINE2MACHINE)
			P(MPT_CHAR, "CombatState")		// Äù½ºÆ® °ÔÀÓ »óÅÂ

		C(MC_QUEST_SECTOR_START, "Quest.Sector.Start", "Quest Sector Start", MCDT_MACHINE2MACHINE)
			P(MPT_CHAR, "SectorIndex")		// »õ·Î¿î ¼½ÅÍ ÀÎµ¦½º
			P(MPT_UCHAR, "RepeatIndex")		// ÇöÀç ¹Ýº¹ È½¼ö (¼­¹ÙÀÌ¹ú¿ë)

		C(MC_QUEST_COMPLETED, "Quest.Complete", "Complete Quest", MCDT_MACHINE2MACHINE)
			P(MPT_BLOB, "RewardInfo");		// º¸»ó ³»¿ë
		C(MC_QUEST_FAILED, "Quest", "Quest failed", MCDT_MACHINE2MACHINE)	//Äù½ºÆ® ½ÇÆÐ.
...
//			P(MPT_VECTOR, "Position")
		...
		C(MC_QUEST_PEER_NPC_SKILL_START,	"Quest.Peer.NPC.Skill.Start",	"NPC Skill Start", MCDT_PEER2PEER)
			P(MPT_UID, "uidOwner")		// ¾´ NPC
			P(MPT_INT, "nSkill")		// ½ºÅ³ ¹øÈ£
			P(MPT_UID, "uidTarget")		// target
			P(MPT_POS, "targetPos")		// À§Ä¡
		...
		C(MC_NEWQUEST_PEER_NPC_BOSS_HPAP,		"NewQuest.Peer.NPC.Boss.HpAp",		"NPC Boss HpAp", MCDT_PEER2PEER)
			P(MPT_UID, "uidBoss")
			P(MPT_FLOAT, "Hp")
			P(MPT_FLOAT, "Ap")
		...
		C(MC_NEWQUEST_ROUND_START,	"NewQuest.Round.Start", "NewQuest Start Round", MCDT_MACHINE2MACHINE)
			P(MPT_UCHAR, "round")

		C(MC_NEWQUEST_PLAYER_DEAD, "NewQuest.PlayerDead", "NewQuest Player Dead", MCDT_MACHINE2MACHINE)
			P(MPT_UID, "Victim");

		C(MC_NEWQUEST_REQUEST_DEAD, "NewQuest.RequestDead", "NewQuest Request Dead", MCDT_MACHINE2MACHINE)
...
*/
		...
		// ¿ì¼±Àº º¸¿©Áö´Â °ÍµéÀ» À§ÇÑ...Á¡Â÷Àû ÀÎÀÚ Ãß°¡..
		// ½Ã°£µîÀÇ µ¿±âÈ­ °ü·Ã ÀÎÀÚ´Â ³ªÁß¿¡ Ãß°¡..
		...
		C(MC_PEER_NOLEAD_RANGE, "Peer.NoLead.Range", "NoLead.Range", MCDT_PEER2PEER)
			P(MPT_BLOB, "RangeInfo");

		C(MC_PEER_NOLEAD_MELEE, "Peer.NoLead.Melee", "NoLead.Melee", MCDT_PEER2PEER)
			P(MPT_BLOB, "MeleeInfo");
...
*/
	...
	// MatchServer, Agent, Client °ø¿ë ¸í·É¾î
...
//	CA("connect", "Net.Connect");
	...
	C( MC_MATCH_USER_REWARD_QUEST, "Quest", "Quest reward.", MCDT_MACHINE2MACHINE )
		...
		// P( MPT_BLOB, "Monster bible info" ) »ç¿ëÇÏÁö ¾Ê±â·Î ÇÔ.
	...
	C( MC_MATCH_RESPONSE_DROP_SACRIFICE_ITEM, "Quest", "Response drop sacrifice item", MCDT_MACHINE2MACHINE )
		...
		// P( MPT_UID, "owner of was drop." )	// MUID(0, 0)ÀÌ¸é ºó ½½·Ô. ¹æÀåÀÏ °æ¿ì¸¸ »ç¿ë.
		P( MPT_INT, "Slot index" )
		P( MPT_INT, "ItemID" )
	...
	C( MC_MATCH_RESPONSE_CALLBACK_SACRIFICE_ITEM, "Quest", "Request callback sacrifice item", MCDT_MACHINE2MACHINE )
		...
		// P( MPT_UID, "owner of was drop" )	// ¹æÀåÀÏ °æ¿ì¸¸ »ç¿ë.
		P( MPT_INT, "Slot index" )
		P( MPT_INT, "ItemID" )
	...
	C(MC_PEER_KILLSTREAK, "Killstreak", "Killstreak sounds and message.", MCDT_PEER2PEER)
     ...
     C(MC_ADMIN_BAN, "Admin.Ban", "Banned Target Player", MCDT_MACHINE2MACHINE)
         P(MPT_STR, "strTargetName")

     C(MC_ADMIN_GRADES, "Admin.Grades", "Change Target Grade", MCDT_MACHINE2MACHINE)
         P(MPT_STR, "SenderName")
		 P(MPT_STR, "TargetName")
		 P(MPT_INT, "Grade")
