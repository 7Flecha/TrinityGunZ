...
TCHAR g_szDB_CREATE_ACCOUNT_NETMARBLE[] = _T("{CALL spInsertAccount ('%s', '%s', %d, '%s', %d, %d)}");
...
// Parameters: Name, Kills.
TCHAR g_szDB_STREAK_LOG[] = _T("{CALL spStreakLog ('%s', %d)}");

// 클랜 폐쇄 예약
// 인자: CLID, ClanName, MasterCID
TCHAR g_szDB_RESERVE_CLOSE_CLAN[] = _T("{CALL spDeleteClan (%d, '%s')}");
...
TCHAR g_szDB_ADMIN_BAN_UPDATE[] = _T("UPDATE Account SET UGradeID=%d WHERE AID=%d");
...
bool MMatchDBMgr::GetAccountInfo( const unsigned long int nAID, MMatchAccountInfo* poutAccountInfo, const int nServerID )
{
	...
	poutAccountInfo->m_nVipCoins		= (unsigned long)rs.Field("VipCoins").AsLong();
...
bool MMatchDBMgr::InsertStreakLog(const TCHAR* szName, const int nKills)
{
	_STATUS_DB_START;
	if (!CheckOpen()) return false; 
	...
	try {
		strSQL.Format( g_szDB_STREAK_LOG, szName, nKills );
		m_DB.ExecuteSQL( strSQL );
	} 
	catch(CDBException* e) {
...
bool MMatchDBMgr::ReserveCloseClan(const int nCLID, const TCHAR* szClanName, const int nMasterCID, const string& strDeleteDate )
{
	...
	try {		
		strSQL.Format(g_szDB_RESERVE_CLOSE_CLAN, nCLID, szClanName );
		m_DB.ExecuteSQL( strSQL );
	} 
	catch(CDBException* e) {
...
bool MMatchDBMgr::OnAdminBan(const int nAID)
{
    _STATUS_DB_START;
    if (!CheckOpen()) return false;
    ...
    try {
        ...
        strSQL.Format(g_szDB_ADMIN_BAN_UPDATE, nGrade, nAID);
        m_DB.ExecuteSQL( strSQL );
    }
    catch(CDBException* e)
    {
