...
void ZWeaponRocket::Create(RMesh* pMesh,rvector &pos, rvector &dir,ZObject* pOwner) {
	...
	int nummer = 0;
	if(sscanf(ZGetGameClient()->GetStageName(), "!r%i", &nummer))
	{
		...
		{
			float rocket_speed = (nummer * 10.0f);
			m_Velocity=dir*ROCKET_VELOCITY * 0.1f * rocket_speed;
		}
	}
	else
	{
...
MImplementRTTI(ZWeaponMolotovGrenade,ZMovingWeapon);

void ZWeaponMolotovGrenade::Create(RMesh* pMesh,rvector &pos, rvector &velocity,ZObject* pOwner) {
...
bool ZWeaponMolotovGrenade::Update(float fElapsedTime)
{
	rvector oldPos = m_Position;
	if(ZGetGame()->GetTime() - m_fStartTime > GRENADE_LIFE) {
		Explosion();
...
void ZWeaponMolotovGrenade::Explosion()
{
	rvector v = m_Position;
...
///
MImplementRTTI(ZWeaponIceGrenade,ZMovingWeapon);

void ZWeaponIceGrenade::Create(RMesh* pMesh,rvector &pos, rvector &velocity,ZObject* pOwner) {
...
bool ZWeaponIceGrenade::Update(float fElapsedTime)
{
	rvector oldPos = m_Position;
	if(ZGetGame()->GetTime() - m_fStartTime > GRENADE_LIFE) {
		Explosion();
...
void ZWeaponIceGrenade::Explosion()
{
	rvector v = m_Position;
	...
	ZGetGame()->OnExplosionSpecialGrenade(m_uidOwner,v,m_fDamage,400.f,.2f,1.f,m_nTeamID, "ice");
...
/// FIRE
MImplementRTTI(ZWeaponFireGrenade,ZMovingWeapon);

void ZWeaponFireGrenade::Create(RMesh* pMesh,rvector &pos, rvector &velocity,ZObject* pOwner) {
...
bool ZWeaponFireGrenade::Update(float fElapsedTime)
{
	rvector oldPos = m_Position;
	if(ZGetGame()->GetTime() - m_fStartTime > GRENADE_LIFE) {
		Explosion();
...
void ZWeaponFireGrenade::Explosion()
{
	rvector v = m_Position;
	...
	ZGetGame()->OnExplosionSpecialGrenade(m_uidOwner,v,m_fDamage,400.f,.2f,1.f,m_nTeamID, "fire");
...
//POISON
MImplementRTTI(ZWeaponPoisonGrenade,ZMovingWeapon);

void ZWeaponPoisonGrenade::Create(RMesh* pMesh,rvector &pos, rvector &velocity,ZObject* pOwner) {
...
bool ZWeaponPoisonGrenade::Update(float fElapsedTime)
{
	rvector oldPos = m_Position;
	if(ZGetGame()->GetTime() - m_fStartTime > GRENADE_LIFE) {
		Explosion();
...
void ZWeaponPoisonGrenade::Explosion()
{
	rvector v = m_Position;
	...
	ZGetGame()->OnExplosionSpecialGrenade(m_uidOwner,v,m_fDamage,400.f,.2f,1.f,m_nTeamID, "poison");
...
//Lightning
MImplementRTTI(ZWeaponLightningGrenade,ZMovingWeapon);

void ZWeaponLightningGrenade::Create(RMesh* pMesh,rvector &pos, rvector &velocity,ZObject* pOwner) {
...
bool ZWeaponLightningGrenade::Update(float fElapsedTime)
{
	rvector oldPos = m_Position;
	if(ZGetGame()->GetTime() - m_fStartTime > GRENADE_LIFE) {
		Explosion();
...
void ZWeaponLightningGrenade::Explosion()
{
	rvector v = m_Position;
	...
	ZGetGame()->OnExplosionSpecialGrenade(m_uidOwner,v,m_fDamage,400.f,.2f,1.f,m_nTeamID, "lightning");
...
#define MAGIC_KNOCKBACK_CONST   .3f	
	...
	if(type == WMET_OBJECT) {// object 에 부딪히는 경우...
		...
		*/
		...
		case ZSR_GOLD :
			{
				ZModule_GoldDamage *pMod = (ZModule_GoldDamage*)pVictim->GetModule( ZMID_GOLDDAMAGE);
				if ( pMod)  pMod->BeginDamage( NULL, 0, 0.9f);
