...
#define STR_GUNZ_EXCUTE_FILENAME			"GunzBeta.exe"
#define STR_LAUNCHER_EXCUTE_FILENAME		"GunzLauncher.exe"
#define STR_PATCHINFO_FILENAME				"patch.xml"
#define STR_WEBPAGE_URL						"http://patch.devilgunz.com/DGUpdate/Launcher/index.php"
#define STR_LICENSE_URL						"license.htm"
#define STR_ADDRESS_REAL					"patch.devilgunz.com"
#define STR_ADDRESS_ALPHA					"patch.devilgunz.com"
#define STR_DIRECTORY_REAL					"DGUpdate/Gunz"
#define STR_DIRECTORY_ALPHA					"DGUpdate/GunzTest"
#endif LOCALE_NHNUSA
...
CNewAppDlg::CNewAppDlg( CWnd* pParent) : CDDBDialog( IDD_DIALOG_MAINDLG, pParent)
{
	...
	DlgInfo.bUseMaximizeButton = true;
...
BOOL CNewAppDlg::OnInitDialog() 
{
	...
	m_cStartGame.MoveWindow( 256, 250);
...
void CNewAppDlg::UpdateWebLoading()
{
	...
	{
#define SIZE_WEBPAGE_X		392
#define SIZE_WEBPAGE_Y		93
...
void CNewAppDlg::StartGame()
{
	...
	if ( !bRunGameProcess && (curr_time > 1000))
	{
		...
		if ( m_bIsAlpha)
			...
			cEncription.CreateSerialKey( 3);


		// 프로세스 실행
		char szCmdLine[ 4096] = { 0, };

#ifdef LOCALE_NHNUSA2
		sprintf( szCmdLine, ".\\%s \"%s\"", STR_GUNZ_EXCUTE_FILENAME, GetNHNModule().GetAuth().GetAuthStr());
#else
		sprintf( szCmdLine, ".\\%s /LoginInfo %s %s %d", STR_GUNZ_EXCUTE_FILENAME, CNewAppDlg::m_szusername, CNewAppDlg::m_szpassword, 77);
#endif
...
void CNewAppDlg::UpdateInfo()
{
	...
	DrawPanel( 10, 249, 240, 58, strMessageFirst);

	// 현재 다운받는 파일 이름 표시
#ifdef LOCALE_NHNUSA
	//GetScreen()->PutText( 0, 249, 15, "Arial", strMessageSecond, GetTextColor());
#else
	//GetScreen()->PutText( 0, 249, 15, "Arial", strMessageSecond, GetTextColor());
#endif
	...
	// 게이지 바 표시
	float fGausse = 215.0f * (fPercent / 100.0f) + 1.5f * (float)sin( (double)timeGetTime() / 100.0f);
	if ( (fGausse < 0.0f) || (fPercent < 0.01f))		fGausse = 0.0f;
	if ( (fGausse > 215.0f) || (fPercent > 99.99f)) 	fGausse = 215.0f;

	GetScreen()->PutBitmap( 20, 279, GetSkin()->GetDC(), 0, 86, 215, 19);
	GetScreen()->PutBitmap( 20, 279, GetSkin()->GetDC(), 0, 63, (int)fGausse, 19);
...
#ifdef LOCALE_NHNUSA
		GetScreen()->PutText( 10, 279, 240, 19, 15, "Arial", strPercent.GetBuffer(), RGB( 50, 50, 50), TS_NORMAL, DT_CENTER);
		GetScreen()->PutText( 10, 279, 240, 19, 15, "Arial", strPercent.GetBuffer(), RGB( 200, 200, 200), TS_NORMAL, DT_CENTER);
#else
		GetScreen()->PutText( 10, 279, 240, 19, 15, "Arial", strPercent.GetBuffer(), GetTextColor(), TS_NORMAL, DT_CENTER);
#endif
	}


	// 화면 복원
	Rendering( 10, 249, 240, 58);
}
