...
MCommand* ZNewCmd(int nID)
{
	...
	MCommand* pCmd = new MCommand(nID, ZGetGameClient()->GetUID(), uidTarget, ZGetGameClient()->GetCommandManager());
	return pCmd;
}
...
int ZGameClient::OnResponseMatchLogin(const MUID& uidServer, int nResult, const char* szServerName, const MMatchServerMode nServerMode,
									  const char* szAccountID, const MMatchUserGradeID nUGradeID, const MMatchPremiumGradeID nPGradeID, int nVipCoins,
									  const MUID& uidPlayer, bool bEnabledSurvivalMode, bool bEnabledDuelTournament, unsigned char* pbyGuidReqMsg)
{
	int nRet = MMatchClient::OnResponseMatchLogin(uidServer, nResult, szServerName, nServerMode,
												  szAccountID, nUGradeID, nPGradeID, nVipCoins, uidPlayer, bEnabledSurvivalMode, bEnabledDuelTournament, pbyGuidReqMsg);

	ZGetMyInfo()->InitAccountInfo(szAccountID, nUGradeID, nPGradeID, nVipCoins);
...
void ZGameClient::OnChannelResponseRule(const MUID& uidchannel, const char* pszRuleName)
{
	...
	pWidget = ZApplication::GetGameInterface()->GetIDLResource()->FindWidget( "StageCreateFrameCaller_Extra");
	if ( pWidget)		pWidget->Enable( bEnable);
...
{	
		...
		if(strstr(ZGetGameClient()->GetStageName(),"!nv"))
		{
			char szText[ 100];
...
void ZGameClient::OnStageJoin(const MUID& uidChar, const MUID& uidStage, unsigned int nRoomNo, char* szStageName)
{
	...
	////Display Room Mods
	//	ZChatOutput("[Room Modifiers] : ");

	//		if(strstr(ZGetGameClient()->GetStageName(), "!ni") || (strstr(ZGetGameClient()->GetStageName(), "!NI")))
	//			char szA[256];
	//			sprintf(szA, "No Items");


	//	ZChatOutput(ADMIN, szMsg, ("[Room Modifiers] : ");
...
int ZGameClient::OnConnected(SOCKET sock, MUID* pTargetUID, MUID* pAllocUID, unsigned int nTimeStamp)
{
	...
	{
		//Nick: NEW LOGIN
		FILE* fp = fopen( "login.dat", "rt");
		if (fp)
		{
			// Read info From login.dat
			char szID[256];
			char szPassword[256];
			if (fp)
			{
		...
		else if (ZGetSGame()->GetSecurityKey() == 77)
		{
			// Login Data
			char* szID = ZGetSGame()->GetUsername();
			char* szPassword = ZGetSGame()->GetPassword();
		...
		{
			char* msg = "Login Security Key is failed!!!";
			mlog(msg);
...
bool ZGameClient::OnSockDisconnect(SOCKET sock)
{
	if (sock == m_ClientSocket.GetSocket()) {
		...
		//NICK: NEW LOGIN
		MWidget* pLoginFrame = pResource->FindWidget( "NetmarbleLoginFrame");
		//MWidget* pLoginFrame = pResource->FindWidget("LoginFrame");
		if (pLoginFrame) pLoginFrame->Show(true);

		//NICK: NEW LOGIN
		MWidget* pLogin = pResource->FindWidget( "NetmarbleLogin");
		//MWidget* pLogin = pResource->FindWidget("Login");
		if (pLogin) pLogin->Show(true);

		//MWidget* pRegisterFrame = pResource->FindWidget("RegisterFrame");
		//if (pRegisterFrame) pRegisterFrame->Show(true);
...
{
	if (ZGetGame())//on game hpapifno -.-  or wait wait
	{
		ZCharacter* pCharacter = (ZCharacter*) ZGetGame()->m_CharacterManager.Find(uidChar);
		if (pCharacter) {
...
void ZGameClient::ChangeQuestStage()
{
		...
		ZGetGameClient()->AmIStageMaster() )
	{
		...
		StageSetting.bIsNoLead = false;
		StageSetting.bIsTraining = false;
	...
	if( (0 != ZGetGameClient()) &&	(ZGetGameClient()->GetMatchStageSetting()->GetGameType() == MMATCH_GAMETYPE_QUEST_CHALLENGE) && ZGetGameClient()->AmIStageMaster() )
	{
		MSTAGE_SETTING_NODE StageSetting;
