...
int RMain(const char *AppName, HINSTANCE this_inst, HINSTANCE prev_inst, LPSTR cmdline, int cmdshow, RMODEPARAMS *pModeParams, WNDPROC winproc, WORD nIconResID )
{
	...
	{
		//pModeParams->nWidth = GetSystemMetrics(SM_CXSCREEN);
		//pModeParams->nHeight = GetSystemMetrics(SM_CYSCREEN);
	}

	DWORD dwStyle = pModeParams->bFullScreen ? WS_POPUP | WS_SYSMENU : WS_POPUP | WS_CAPTION | WS_SYSMENU ;//(WS_OVERLAPPED | WS_POPUP);
