...
bool RBaseTexture::OnRestore(bool bManaged)
{
	...
	//! Jacob - VERY GHETTO FIX FOR MY MISTAKE!

	if (strstr(m_szTextureName, "Common/Model_data/man/"))
	{
		char szTexture[256];
		memset(szTexture, 0, 256);
		int nLen = strlen("Common/Model_data/Man/");
		strncpy(szTexture, m_szTextureName+nLen, strlen(m_szTextureName)-nLen);
		memset(m_szTextureName, 0, strlen(m_szTextureName));
		sprintf(m_szTextureName, "Common/Model_data/man/%s", szTexture);
		m_szTextureName[strlen(m_szTextureName)] = 0;
	}
	else if (strstr(m_szTextureName, "Common/Model_data/woman/"))
	{
		char szTexture[256];
		memset(szTexture, 0, 256);
		int nLen = strlen("Common/Model_data/woman/");
		strncpy(szTexture, m_szTextureName+nLen, strlen(m_szTextureName)-nLen);
		memset(m_szTextureName, 0, strlen(m_szTextureName));
		sprintf(m_szTextureName, "Common/Model_data/woman/%s", szTexture);
		m_szTextureName[strlen(m_szTextureName)] = 0;
	}
