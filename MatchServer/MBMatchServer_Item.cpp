									...
									, const bool bIsGambleItem, const DWORD dwPrice, const WORD wRentHourPeriod, const bool bIsSpendableItem, int nBuyMode)
{
	...
	pBuyBountyItemJob->Input( pObj->GetUID(), pCharInfo->m_nCID, pAccountInfo->m_nAID, nItemID, nItemCount, dwPrice, wRentHourPeriod, bIsGambleItem, bIsSpendableItem, nBuyMode );
...
const bool MBMatchServer::CheckUserCanBuyItem( MMatchObject* pObj, const int nItemID, const int nItemCnt, const DWORD dwPrice, int nBuyMode )
{
	...
	else if ((bIsCashItem) && (static_cast<DWORD>(pAccountInfo->m_nVipCoins) < dwPrice))
	{
		MCommand* pNew = CreateCommand( MC_MATCH_RESPONSE_BUY_ITEM, pObj->GetUID() );
		pNew->AddParameter(new MCmdParamInt(MERR_TOO_EXPENSIVE_BOUNTY));
		PostSafeQueue( pNew );
	...
	if ((nBuyMode == 2) && !((pAccountInfo->m_nUGrade == MMUG_OWNER) || (pAccountInfo->m_nUGrade == MMUG_GAMEMASTER)  || (pAccountInfo->m_nUGrade == MMUG_FMOD) || (pAccountInfo->m_nUGrade == MMUG_DEVELOPER))) return false;
...
bool MBMatchServer::ResponseCharacterItemList(const MUID& uidPlayer)
{
	...
	pNew->AddParameter(new MCommandParameterInt(pObj->GetAccountInfo()->m_nVipCoins));
