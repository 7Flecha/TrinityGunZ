...
void ChatCmd_AdminFramed(const char* line, const int argc, char **const argv);
void ChatCmd_AdminGod(const char* line, const int argc, char **const argv);
void ChatCmd_AdminBan(const char* line, const int argc, char **const argv);
void ChatCmd_AdminSenditem(const char* line, const int argc, char **const argv);
void ChatCmd_TestChangeWeapon(const char* line, const int argc, char **const argv);

//custom Player
void ChatCmd_Texture(const char* line, const int argc, char **const argv);
void ChatCmd_Damagelog(const char* line, const int argc, char **const argv);
void ChatCmd_Colors(const char* line, const int argc, char **const argv);
...
{	
	...
	{
		MLog("Error while Read Item Descriptor %s", "localized/localize_english/chatcmds.xml");
	}
...
//	_CC_ALIAS("종료", "admin_halt");
	...
	_CC_AC("admin_wireframe",						&ChatCmd_AdminFramed,						CCF_ADMIN|CCF_GAME, ARGVNoMin, ARGVNoMax, true, "/admin_wireframe", "");
	_CC_AC("admin_god",                             &ChatCmd_AdminGod,                          CCF_ADMIN|CCF_GAME, ARGVNoMin, ARGVNoMax, true, "/admin_god", "");
	_CC_AC("admin_ban",                             &ChatCmd_AdminBan,                          CCF_ADMIN|CCF_GAME, ARGVNoMin, ARGVNoMax, true, "/admin_ban <charname>", "");
	_CC_AC("admin_changeweapon",					&ChatCmd_TestChangeWeapon,					CCF_ADMIN|CCF_GAME, ARGVNoMin, 2,		  true, "/admin_changeweapon <PartId> <ItemId>", "");

	// Extra cmds
	_CC_AC("Damagelog",                             &ChatCmd_Damagelog,                         CCF_ALL|CCF_GAME, ARGVNoMin, ARGVNoMax, true, "/damagelog", "");
	_CC_AC("Colors",								&ChatCmd_Colors,							CCF_ALL|CCF_GAME, ARGVNoMin, ARGVNoMax, true, "/colors", "");
	_CC_AC("Color",									&ChatCmd_Colors,							CCF_ALL|CCF_GAME, ARGVNoMin, ARGVNoMax, true, "/color", "");
	_CC_AC("Colours",								&ChatCmd_Colors,							CCF_ALL|CCF_GAME, ARGVNoMin, ARGVNoMax, true, "/colours", "");
...
void ChatCmd_AdminPopup(const char* line, const int argc, char **const argv)
{
	// Check For Allowed player ranks
	SGame::IsImportantPlayer();
...
void ChatCmd_AdminFramed(const char* line, const int argc, char **const argv)
{
	...
	if(state == GUNZ_GAME)
	{
		if(ZGetSGame()->bWireFrame == false)
		{
			ZGetSGame()->bWireFrame = true;
			sprintf(szMsg, "^1Wireframe mode activated!");
			ZChatOutput(szMsg);
		}
		else if(ZGetSGame()->bWireFrame == true)
		{
			ZGetSGame()->bWireFrame = false;
...
void ChatCmd_AdminGod(const char* line, const int argc, char **const arv)
{
	// Check For Allowed player ranks
	SGame::IsImportantPlayer();
    ...
    if(state == GUNZ_GAME)
    {
        if(ZGetSGame()->bGodMode == false)
        {
            ZGetSGame()->bGodMode = true;
            sprintf(szMsg, "^1God enabled!");
        }
        else if(ZGetSGame()->bGodMode == true)
        {
            ZGetSGame()->bGodMode = false;;
            sprintf(szMsg, "^1God disabled!");
        }
    }
    else
    {
...
void ChatCmd_TestChangeWeapon(const char* line, const int argc, char **const argv)
{
	...
	int ItemPart = atoi(argv[2]);

	ZMyCharacter* m_pMyCharacter = ZGetGame()->m_pMyCharacter;
	m_pMyCharacter->GetItems()->EquipItem((MMatchCharItemParts)ItemPart, ItemID);

	switch(ItemPart)
	{
		case 0: m_pMyCharacter->ChangeWeapon(MMCIP_HEAD); break;
		case 1: m_pMyCharacter->ChangeWeapon(MMCIP_CHEST); break;
		case 2: m_pMyCharacter->ChangeWeapon(MMCIP_HANDS); break;
		case 3: m_pMyCharacter->ChangeWeapon(MMCIP_LEGS); break;
		case 4: m_pMyCharacter->ChangeWeapon(MMCIP_FEET); break;
		case 5: m_pMyCharacter->ChangeWeapon(MMCIP_FINGERL); break;
		case 6: m_pMyCharacter->ChangeWeapon(MMCIP_FINGERR); break;
		case 7: m_pMyCharacter->ChangeWeapon(MMCIP_MELEE); break;
		case 8: m_pMyCharacter->ChangeWeapon(MMCIP_PRIMARY); break;
		case 9: m_pMyCharacter->ChangeWeapon(MMCIP_SECONDARY); break;
		case 10: m_pMyCharacter->ChangeWeapon(MMCIP_CUSTOM1); break;
		case 11: m_pMyCharacter->ChangeWeapon(MMCIP_CUSTOM2); break;
		case 12: m_pMyCharacter->ChangeWeapon(MMCIP_AVATAR); break;
		case 13: m_pMyCharacter->ChangeWeapon(MMCIP_COMMUNITY1); break;
		case 14: m_pMyCharacter->ChangeWeapon(MMCIP_COMMUNITY2); break;
		case 15: m_pMyCharacter->ChangeWeapon(MMCIP_LONGBUFF1); break;
		case 16: m_pMyCharacter->ChangeWeapon(MMCIP_LONGBUFF2); break;
	}
}
///////////////////////////////////////////////////////////////////////////////
// Players
void ChatCmd_Damagelog(const char* line, const int argc, char **const arv)
{
	GunzState state = ZApplication::GetGameInterface()->GetState();
    char szMsg[256];
    if(state == GUNZ_GAME)
    { 
        ...
        if(ZGetSGame()->bDamageCounter == false)
        {
            ZGetSGame()->bDamageCounter = true;
            sprintf(szMsg, "^1Damage Log On");
        }
        else if(ZGetSGame()->bDamageCounter == true)
        {
            ZGetSGame()->bDamageCounter = false;
            sprintf(szMsg, "^1Damage Log Off!");
        }
		ZChatOutput(szMsg);
		return;
    } 
    ...
    {
        ZChatOutput("You must be in-game!");
    }
}
void ChatCmd_Colors(const char* line, const int argc, char **const arv)
{
    char szMsg1[256];
	char szMsg2[256];
	char szMsg3[256];
	char szMsg4[256];
	sprintf(szMsg1, "^00 ^11 ^22 ^33 ^44 ^55 ^66 ^77 ^88 ^99");
	sprintf(szMsg2, "^aa ^bb ^cc ^dd ^ee ^ff ^gg ^hh ^ii ^jj");
	sprintf(szMsg3, "^kk ^ll ^mm ^nn ^oo ^pp ^qq ^rr ^ss ^tt");
	sprintf(szMsg4, "^uu ^vv ^ww ^xx ^yy ^zz");
	ZChatOutput(szMsg1); 
	ZChatOutput(szMsg2); 
	ZChatOutput(szMsg3);
	ZChatOutput(szMsg4);
