...
bool ZApplication::ParseArguments(const char* pszArgs)
{
	...
	// Login Information
	#define ZTOKE_GAME_LOGININFORMATION		"LoginInfo"
	char *str;
	str=(char*)strstr(pszArgs, ZTOKE_GAME_LOGININFORMATION);
	if ( str != NULL) 
	{
		char szUser[256], szPass[256];
		int nSecurityKey = 0;
		sscanf(str, "%s %s %d", szUser, szPass, &nSecurityKey);
		bool bShotEnable = false;
		if (nSecurityKey != 0)
		{
			ZGetSGame()->SetAllInfo(szUser, szPass, nSecurityKey);
		}
		return true;
	}
	...
	case MC_US:
	case MC_INDIA:
//	case MC_BRAZIL: //Nick: Delete Local is brz
		{
			// 암호 코드 구하기
			CGLEncription cEncription;
			int nMode = cEncription.Decription();
	...
	case MC_BRAZIL: //Nick: add for brz local
	case MC_BETA:
		{
			SGame::Launcher();
			if(SGame::bAllowed == 0)
...
//	if(m_MeshMgr.LoadXmlList("common/model_mat/character_lobby.xml")==-1) return false;
	if(m_MeshMgr.LoadXmlList("common/model_mat/character.xml",ZProgressCallBack,&meshLoading)==-1)	
		return false;
...
#ifdef _QUEST
	//if(m_NPCMeshMgr.LoadXmlList("common/model_mat/npc.xml",ZProgressCallBack,&npcLoading) == -1)
	if(m_NPCMeshMgr.LoadXmlList("common/model_mat/npc.xml") == -1)
		return false;
#endif
...
//	npcLoading.UpdateAndDraw(1.f);
	...
	string strFileNameWeapon("common/weapon_mat/weapon.xml");
#ifndef _PUBLISH //Was _DEBUG to disable mef files
...
//	mlog("Init effect manager success.\n");
	...
	if (!MGetMapDescMgr()->Initialize(GetFileSystem(), "localized/localize_english/map.xml"))
	{
		MLog("Error while Read map Descriptor %s\n", "localized/localize_english/map.xml");
	}
	mlog("Init map Descriptor success.\n");


	string strFileChannelRule("localized/localize_english/channelrule.xml");
	...
	// if (!MGetChattingFilter()->Create(GetFileSystem(), "localized/localize_english/abuse.xml"))
...
bool ZApplication::InitLocale()
{
	...
	char szPath[MAX_PATH] = "localized/localize_english/";
