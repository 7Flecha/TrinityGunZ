	...
	case MWM_HOTKEY:
		{
			...
			ZApplication::GetGameInterface()->GetChat()->Input(buffer, 1337);
					...
					if(str) {
						if(ZApplication::GetGameInterface())
							if(ZApplication::GetGameInterface()->GetChat())
								ZApplication::GetGameInterface()->GetChat()->Input(str, 1337);
					}
				}
				return true;
			...
			case VK_F9:
				...
				else
				{
					if(ZGetGame()) {
						...
						int FinalHP = floor(fHP + 0.5);
						int FinalAP = floor(fAP + 0.5);
						sprintf(szName, ZGetMyInfo()->GetCharName());
						sprintf(HPA,"%d", FinalHP);
						sprintf(APA,"%d", FinalAP);
						MMATCH_GAMETYPE GameType = ZGetGameClient()->GetMatchStageSetting()->GetGameType();
						if(!ZGetGame()->m_pMyCharacter->IsDie()) {
						if(GameType == MMATCH_GAMETYPE_DEATHMATCH_TEAM || GameType == MMATCH_GAMETYPE_GLADIATOR_TEAM || GameType == MMATCH_GAMETYPE_ASSASSINATE || GameType == MMATCH_GAMETYPE_DEATHMATCH_TEAM2 || GameType == MMATCH_GAMETYPE_TRAINING_TEAM)
						{
							int nTeam = MMT_ALL;
							nTeam = ZGetGame()->m_pMyCharacter->GetTeamID();
							sprintf(FinishStr, "(%s) HP: %s AP: %s", szName, HPA, APA);
							ZPostPeerChat(FinishStr,  nTeam);
							F9Timer = clock () + 3 * CLOCKS_PER_SEC ;
						} else {
							sprintf(FinishStr, "(%s) HP: %s AP: %s", szName, HPA, APA);
							ZPostPeerChat(FinishStr,  MMT_ALL);
							F9Timer = clock () + 3 * CLOCKS_PER_SEC ;
						}
						}
					}
	...
	case MWM_SYSKEYDOWN:
		{
			...
			/*if(pEvent->nKey==90){	// Alt+'Z' // 모든 UI 감추기... by kammir 20081020
				char szName[128];
				{
					...
					{
						sprintf(szName, "^1HP: ^0%d / %d ^2AP: ^0%d / %d", (int)ZGetGame()->m_pMyCharacter->GetHP(), (int)ZGetGame()->m_pMyCharacter->GetMaxHP(), (int)ZGetGame()->m_pMyCharacter->GetAP(), (int)ZGetGame()->m_pMyCharacter->GetMaxAP());
						ZApplication::GetGameInterface()->GetChat()->Input(szName, 1337);
						return true;
					}
					else
					{
			...
			//{
			//	char szTemp[1337];
			//	mlog("Pos = %6.3f %6.3f %6.3f  Dir = %6.3f %6.3f %6.3f \r", ZGetGame()->m_pMyCharacter->GetPosition().x, ZGetGame()->m_pMyCharacter->GetPosition().y, ZGetGame()->m_pMyCharacter->GetPosition().z, ZGetGame()->m_pMyCharacter->m_Direction.x, ZGetGame()->m_pMyCharacter->m_Direction.y, ZGetGame()->m_pMyCharacter->m_Direction.z);
			//	sprintf(szTemp, "Pos = %6.3f %6.3f %6.3f  Dir = %6.3f %6.3f %6.3f", ZGetGame()->m_pMyCharacter->GetPosition().x, ZGetGame()->m_pMyCharacter->GetPosition().y, ZGetGame()->m_pMyCharacter->GetPosition().z, ZGetGame()->m_pMyCharacter->m_Direction.x, ZGetGame()->m_pMyCharacter->m_Direction.y, ZGetGame()->m_pMyCharacter->m_Direction.z);
			//	ZPostPeerChat(szTemp,  MMT_ALL);
			//	return true;
			//}
